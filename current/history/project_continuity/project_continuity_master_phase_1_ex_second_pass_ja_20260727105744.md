...
updated_at: 2026-07-27 10:55:01 JST
...
Lightning Auto-start         : STAGE A／B REPOSITORY PREPARATION ACCEPTED
Traffic-aware Wake-up        : PLATFORM MANUAL VALIDATION WAITING
...
現在はGit開始前のDocumentation再編、Phase 1／Phase 1-ex Lossless再整理、Canonical／Public Docs、Identity／License／Termsおよび継続性の整備を行う。Phase 1-exは進行中であり、現時点の再整理版を完了済みFrozen版と誤認させない。
...
1. Documentation Reconstruction Source Inventory／Delta管理
2. Project Continuity Master／Roadmap第1周
3. Current Canonical一式のLossless再構築
4. Phase 1 Frozen Compilation再検証
5. Phase 1-ex Current-to-date Lossless再整理
6. Shared一式のLossless再整理
7. Project Continuity Master／Roadmap第2周（完了）
8. Overview／Concept／README（初版作成済み）
9. LICENSE／TERMS_OF_USE／NOTICE／CITATION（Research Preview初版作成済み）
10. Local Mac用簡易Documentation RAG＋External Hook
11. Git運用設計
12. Initial Commit前Documentation Refresh
13. Sanitation／Allowlist／Git初期化
14. Lightning Traffic-aware Wake-up Manual Validation
15. Public Demo最終確認
16. Phase 1-ex Review／Backup／GitHub公開

Current／Publicの英語派生版は、今回の第1 Documentation Corpusでは作成しない。Phase 1-ex後半のInitial Commit前全体Refreshで、日本語正本と同じ粒度の英語版を再検討する。
...
- Current／Publicは日本語正本を必須とする。英語派生版を作る場合は同じ粒度とし、Phase／Sharedは日本語のみとする。英語版の作成時期はPhase 1-ex後半でユーザーが再判断する。
...
- Lightning Repository側のAuto-start Stage A／B準備はAcceptedだが、Browserを閉じStudioがSleep中でも第三者のPublic URL Accessで起動できるTraffic-aware Wake-upは未検証である。
...
GitはPhase 1-ex後半で開始する。Initial Commit前にCurrent／Public、承認済みLanguage Scope、README、License／Terms、Setup、Public Demo、RAG、Identity、Sanitation、Allowlist、Hash、LinkおよびManifestを再確認する。最初のGitHub公開はPhase 1-ex完了後とする。
...
9. Link、Identity、Secret、Permission、Warrantyおよび情報欠落を検証する。（進行中）

未許可／未実施：

- Git初期化／Commit／Tag／Remote／Push
- Public Repositoryへの投入
...
```text
README.md:
  286d6fba8d0ed6a2c9471db99db7305b1158606893080c0455fbe0379aefce28f906436f4f81b910109e79d02b32c4a0c117097dcfa38fd0cf4a6034182c5517
...
```text
Documentation Corpus Link／Identity／Secret Validation
Public Allowlist／Sanitation
Mac限定簡易Documentation RAG＋External Hook
Lightning Traffic-aware Wake-up Manual Validation
Public Demo Profile／Cost Protection
Git運用設計
Git初期化／Initial Commit準備
Phase 1-ex Final Lossless Compilation
Design Governance Recovery Manifest
Phase 1-ex Final Review／User Acceptance
Phase 1-ex Backup
GitHub Initial Publication
```

現在のPublic文書が人に見せられる粒度であることと、GitHub公開操作が許可・完了していることは別である。Git操作、外部公開、Lightning Platform操作およびPhase完了宣言は、ユーザーの明示許可と各Gate完了まで行わない。
