...
Phase 1-F Lightning Pure CPU Repository Follow-up: Complete／Accepted
...
一方、Phase 1全体はまだ完了宣言前である。Lightning Pure CPUのRepository CorrectionはAcceptedとなったが、Lightning AI Studio上でのNative Build／Model Smoke、公開URL、Cross-environment Final ReviewおよびBackupが残っている。
...
Model選択は`--model-root`を正本とし、RegistryのRelative Artifact Pathから実Fileを解決する。互換用`--model-path`はExpected Layoutとの一致確認だけに使用し、指定Fileと実際にLoadするFileが異なる状態をFail Closedで拒否する。Repository Correctionは独立ReviewでAcceptedとなり、次はユーザーによるExternal Native Gateである。

既存の`preflight_lightning_ai_studio.sh`は、後方互換を維持したうえでPure CPU Targetへ拡張する。既存`--cpu-only`はCUDA BuildのCPU実行を意味するため、Pure CPUへ意味を変更しない。Pure CPU Targetでは`nvidia-smi`、`nvcc`、CUDA CompilerおよびGPU Allocationを確認せず、Linux、x86_64、Container、Python、`uv`、CPU、Memory、PathおよびPure CPU ProfileをRead-onlyで確認する。外部Lightning環境の再構築とNative Testはユーザーが実行し、Repository側は再構築手順とPreflightを提供する。
