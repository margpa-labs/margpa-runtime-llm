...
Canonical／Lossless／Public Docs       : In Progress
Lightning Auto-start Stage A／B Prep  : Repository Accepted
...
Lightning AI Studioでは、Ubuntu 24.04系Linux x86_64 Container、Python 3.12.11、Pure CPU Backend、Qwen3-4B GGUFを用いた環境再構築、Environment Verification、Full Test Suite、Model Acceptance、外部BrowserからのBasic Preview、生成、停止、New Chat、Language、Summary、Thinking、Copy、Busy表示およびLifecycle操作を確認した。Basic Previewは認証付きの手動起動環境としてAcceptedである。

Phase 1は完了宣言と確定Backupまで完了した。現在はPhase 1-exで、Docs Directory Migration後のCanonical文書、Phase 1／Phase 1-ex Lossless文書、Shared運用文書、公開文書および初回公開用Metadataを再構築している。Source InventoryはDocs 493件とDemo画像6件をFreezeし、全499件のPath、SizeおよびSHA-512を検証済みである。

Lightning Auto-startについては、Repository側のRead-only Preflight、Stage A確認、Stage B用Entrypointおよび手順準備までAcceptedである。ただし、本来の目的である「StudioやBrowserを開いていない状態でも、第三者のURL Accessを契機に起動するTraffic-aware Wake-up」はLightning Platform上の手動実試験待ちであり、成立済みとは扱わない。匿名Public Demo、Rate Limit、Cost保護、Tool／RAG遮断を含むPublic Demo Profileもまだ未実装・未公開である。
...
2026年7月27日時点で、Docs Directory Migration、旧Root退役、Migration Manifest、Rollback Plan、役割権限再整理、Stable／History命名規則、Append-Only運用、設計統括者役Recovery方針、Lightning Basic Preview Lifecycle、Auto-start Stage A／B Repository Preparationおよび全Source Inventoryは実施済みである。

現在は、次の順序でDocumentationを再構築している。
...
6. `project_continuity_master_ja.md`と`roadmap_ja.md`を第2周で再確認し、Public Corpus、READMEおよび公開Metadataへ反映する。
7. 全Snapshot、SHA-512、Source Coverage、Link、禁止情報、State表記を検証する。
...
- README／LICENSE／CITATION／NOTICE／TERMS_OF_USE候補
...
最終FilenameはPhase 1-exのDirectory MigrationとCanonical Docs設計で確定する。
...
開発余力がある場合に限り、Phase統合文書と公開文書の英語版を`_en`で作成できる。

- 英語版作成はPhase 1-exまたは各Phase完了の必須Gateにしない。
...
Public Documentation Corpusの準備は、RAG実装または自動有効化を意味しない。Phase 1-exでは安全な入力集合と再現可能なManifestを整備し、Runtime接続は後続Phaseで行う。
...
将来、ユーザーが別Projectで作成した`TERMS_OF_USE.md`および`NOTICE.md`を提示した場合、再利用可能な条項を候補SourceとしてReviewできる。ただし、Project名、対象範囲、利用許諾、禁止事項、免責、責任制限、第三者License、Model LicenseおよびHosted Service条件との整合を確認し、無検証でそのまま流用しない。
...
- Git運用、Git初期化、初回CommitおよびGitHub公開は、Docs最終確認後に別途承認・実施する。
...
本格RAGに先立ち、公開日本語正本を参照して「このProjectは何か」「現在どこまで動くか」「どのPhaseで何を作るか」をLLM自身に説明させる軽量機能を、Phase 1-ex完了後のOptional Early Featureとして予約する。
...
READMEへの表示は実装状態と一致させる。

- 未実装時は、将来予定としてのみ記載する。
- 実装とAcceptance完了後に限り、「このProjectについて、公開Docsを参照しながらLLM自身に説明させることができます」と記載できる。
- Source表示や既知の限界を併記し、Project全体を完全に理解しているとは主張しない。

早期実装は、Phase 1-exの公開文書整備、Privacy Scan、Phase 1完了または主要Gateを遅延させない範囲に限る。工数、Dependency、Test範囲が膨らむ場合はPhase 2またはPhase 7まで延期する。
...
初期実装はMac Local Profileを対象とし、明示的にONにした場合だけCorpus Load、RetrievalおよびContext Injectionを行う。Lightning Profileでは当面HookとConfig Schemaだけを配置し、Default `OFF`、Provider未接続を許容し、Index Load、Retrievalおよび追加Model Callを行わない。これにより、外部環境へ機能の存在や依存を強制せず、後から同じPortへ実装を接続できる。
