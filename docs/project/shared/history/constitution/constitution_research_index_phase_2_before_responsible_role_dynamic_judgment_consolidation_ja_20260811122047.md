...
## 6. General Hard-code Prohibition／Portability

ユーザーが明示した最上位規則として、通常運転、Automation、Constitution、全設計、全Role、全Task、全Agent、全Toolおよび全Providerは、可変要素を再利用されるCoreへ可能な限りHard-codeしてはならない。

ConstitutionのNormative Coreへ特定Project、Repository、Absolute Path、Phase番号、Task名、Role Binding、固定Artifact Package／件数、Threshold、Provider、Vendor、Tool、Command、UI、EnvironmentまたはCloudをHard-codeしない。
...
Hard-codeは、技術的または論理的に不可避で、同等の抽象化手段では契約を維持できない場合に限る。理由、代替案、代替不能性、Exact Scope、Owner、変更・Review方法、除去／Migration条件、TestおよびEvidenceを必要とする。Project Manifest、Authorization Envelope、Role View、ConfigまたはFreeze EventによるExact Runtime Bindingは、Coreへの固定埋込みと区別する。
