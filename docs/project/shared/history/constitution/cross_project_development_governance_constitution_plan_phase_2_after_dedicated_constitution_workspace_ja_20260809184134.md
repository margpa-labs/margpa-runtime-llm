...
現時点では憲法書そのものではなく、作成時期、入力、構造、Portable Package要件およびAcceptanceを固定する予約である。本書だけを根拠に既存規則を置換、要約、廃止または移動しない。専用入口は[Constitution Research Index](../constitution/constitution_research_index_ja.md)、制度候補台帳は[Constitution Source Evidence Register](../constitution/constitution_source_evidence_register_ja.md)とする。

## 2. 作成時期

統合憲法書は、原則としてRuntimeのAgent／Toolを本格実装する前の独立Gateで作成する。Rule Conflict、Provider間解釈差、Source増加によるLossless再構築RiskまたはUser Decisionがある場合は前倒しできる。
...
現在から正式編纂まで、専用`constitution/`と`history/constitution/`へSource Trace、Evidence、Conflict、非採用案および制度候補を累積する。
...
```text
Constitution Folder       : created／formal chapter artifacts not compiled
Constitution Research Folder: created／active
Constitution History      : created／append-only
