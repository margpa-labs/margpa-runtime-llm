...
「最上位規則群」は現時点の列挙で閉じない。Project運用、Pilot、Incident、Near Miss、Agent／Tool設計またはProvider併用から、新しい最上位候補が得られる可能性を明示的に保持する。
...
新候補はEvidence、既存規則とのConflict、適用対象、検知可能性、違反時動作、Recovery、移植性およびUser Decisionを経て追加する。RoleやAutomationが自己判断で最上位規則を作成、変更または免除してはならない。
