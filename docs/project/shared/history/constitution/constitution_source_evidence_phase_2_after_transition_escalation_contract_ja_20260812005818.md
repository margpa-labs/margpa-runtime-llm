...
Current Blockerは、現在Transitionへの直接必要性、現在未解決、委譲Authority内で解決不能、および放置時の安全性・完全性・可逆性・Evidence Integrity・Authority破壊という全条件を満たす場合だけ成立する。`Unresolved`、Evidence保持、過去の失敗または将来の昇格条件だけではBlockerにならない。

未解決事項を`CURRENT_BLOCKER／RESPONSIBLE_ROLE_OWNED_WORK／DEFERRED_EVIDENCE／USER_GATE`へ分離する。最高責任者役と各Roleは自Authority内の解消作業を自分で閉じ、Authority／Scope／Root拡張、External／不可逆Action、要求変更、重大Risk受容、Final Acceptance、Backupまたは最上位規則等のHuman-only事項だけをUserへ返す。

Accepted／Closedその他の確定済みHistorical Outcomeは、Current Transitionを変える新Evidence、Integrity Mismatch、上位規則ConflictまたはUserの明示的再OpenなしにActive Governance Stateへ戻さない。

Closure時は最高責任者役が`GO／ADJUST／STOP`を自分で提案し、Technical Blocker、Role-owned Work、Deferred Evidence、ValidationおよびHuman-only Actionを分離する。Human Decision Burden Minimizationは安全境界の省略ではなく、Human-onlyでない判断を適切なRoleが引き受けるAutomation Acceptance Criterionである。
