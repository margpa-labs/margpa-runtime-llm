...
General Hard-code Prohibitionの現行Normative本文と判断Authorityは[Task Role／Write Authority Policy](../task_roles/task_role_write_authority_policy_ja.md)を正本とする。本Indexは同じ条文を再掲せず、将来のConstitution編纂時に、Portable Core、Project Manifest、Provider Adapter、Constitution ViewおよびRuntime BindingへLosslessに分離するSource Traceだけを保持する。

許可範囲内の抽象化方法、Hard-code不可避性、EvidenceおよびTestは、その時点の最高責任者役が都度判断する。Constitutionは判断を機械的Resolverへ置換せず、人間専有の最上位規則改変と、最高責任者役が担う通常の運用判断を分離する。
