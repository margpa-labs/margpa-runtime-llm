...
### CONST-SRC-014 — General Hard-code Prohibition
...
### CONST-SRC-015 — Dynamic Documentation Requirement Resolution
...
classification:
  ...
  - portability
source:
  - ../task_roles/role_authority_matrix_ja.md#7-dynamic-documentation-requirement-resolution
  - ../operations/documentation_structure_and_task_operations_ja.md#13-task間情報伝達
  - ../automation/automation_governance_evidence_log_ja.md#oge-p2pilot-013--固定document-packageはhard-codeと過剰生成を生む
```

Work Unitへ固定のIndex／Handoff／Status／Review Packageを一律生成せず、Work Unit種別、Role／Task境界、State Transition、Mutation Risk、Review／Human Gate、Audit／Recovery要件およびProvider Capabilityから必要Artifactだけを動的に解決する。

一つのArtifactが複数責務をLosslessに満たせる場合は統合し、必要性を示せないArtifactは作らない。Project Bindingが許可Document Root／Classを与え、Work Unit開始時にResolver結果をExact Class／PathへFreezeする。ResolverはAuthorityを生成せず、既存Stableへの直書き、既存History Mutation、許可外Document ClassまたはAuthorized Root外を許可しない。
...
- Dynamic Documentation Requirement Resolverの判定Schema、説明可能性、過少／過剰生成TestおよびProvider差。
