...
当面、現在Taskは`プロジェクト責任者兼設計統括者役`として両責務を兼務する。伝達段数だけを増やす独立した設計統括者Taskは新設しない。ただし本書とProject Responsibility Handoff／Recoveryは統合・置換せず、それぞれの責務とRecoveryを分離したまま相互参照する。Pilot開始前はTask名を変更せず、正式Start Event成立直後にProvider Capabilityが許す場合だけ変更する。
...
18. `docs/project/shared/automation/automation_governance_index_ja.md`
19. `docs/project/shared/automation/automation_control_profile_ja.md`
...
Phase 2        : STARTED／PHASE 2-0 PILOT DESIGN REVIEW PENDING
Git／GitHub    : EXISTING HISTORY PRESERVED／MAIN ALIGNED AT 30D347E／SINGLE CANONICAL ROOT
```
...
Git運用は[Git Workflow Policy](../operations/git_workflow_policy_ja.md)でAcceptedである。`margpa-runtime-llm`が単一Canonical Git Working Rootであり、Phase 1-ex Closure時の`main`／`origin/main`／Remote `main`は`30d347e0ce05dd208898a4f876e54139cdcacbda`で一致する。ただし、Commit、Push、Tag、Merge、Release、Branch削除またはRemote変更は各対象のユーザー明示承認なしに行わない。
...
Phase 2以降、現設計統括者役をProject責任者とする。責任範囲はProject全体、Cross-Phase不変条件、Phase担当Taskの編成、設計／実装Handoff、Review、RecoveryおよびPhase Closure準備である。Project責任者も絶対禁止事項、Docs規則、Authority規則その他の運用ルールに完全に従属し、自己免除または自己Authority拡張を行わない。
...
Agent／Tool本格実装前に、運用規則をProvider-neutralな統合憲法体系へLossless Compilationする。予定Rootは`docs/project/shared/constitution/`であり、単一巨大Markdownではなく、正本Index、章別Rule、Rule ID、Manifest、Role別Constitution View、SchemaおよびCodex Desktop／Claude Code Provider Adapterから成るPortable Packageを目標とする。
...
## 22. Phase 2開始／Automation Pilot Design State

ユーザーはGitHub、最新DocsおよびBackupを確認し、Phase 1-ex完了とPhase 2開始を明示した。Phase 2は開始済みであるが、現在は`Phase 2-0 Automation Pilot Design`のReview待ちであり、独立Task、Pilot実行および元来のPhase 2機能実装は未開始である。
...
- [Bootstrap Handoff Draft](../../phases/phase_2/handoffs/phase_2_0_phase_designer_bootstrap_handoff_draft_ja.md)
...
初回候補は、Phase 2設計担当者役Taskを最大1件だけ作成して行うRead-onlyの`P2-0-WU-001 Docs-only Recovery and Authority Acknowledgement`である。File、Git、External、Secret、Destructive ActionまたはSub-agent Authorityは与えない。

次のGateは、ユーザーによる設計Review、Authorization Envelopeの明示承認および独立Task作成の明示指示である。これらが成立するまでTaskを作成せず、安全停止する。

## 23. Phase 2 Final Closure／Phase 3 READY Successor State

本節は、旧Phase 2-0開始記録を削除せず、現在状態を上書きするSuccessorである。

```text
Phase 2                  : COMPLETE／ACCEPTED／CLOSED
Phase 2-E                : COMPLETE／USER ACCEPTED
Phase 2-F                : COMPLETE
Lightning                : PHASE 3へ正式延期／Phase 2非Blocker
Phase 3 Design           : ACCEPTED／FROZEN
Phase 3 State            : READY／NOT STARTED
Automation Control       : OFF
Implementation Authority: FALSE
Next Required Gate       : Start時Preflight → Codex ARMED → User Explicit Start
```

新しい設計統括者役は[Phase 3 Index](../../phases/phase_3/phase_index_ja.md)と[Phase 3 Frozen／READY Handoff](../../phases/phase_3/handoffs/phase_3_claude_execution_handoff_ja.md)を読み、Phase 2を再実行しない。`READY`を`ARMED`、Automation `ON`または実装開始と解釈しない。
