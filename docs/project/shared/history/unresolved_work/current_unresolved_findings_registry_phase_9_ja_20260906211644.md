...
last_reclassified_at: 2026-09-06 21:16:43 JST
last_updated_at: 2026-09-06 21:16:43 JST
...
history_snapshot: ../history/unresolved_work/phase_9_1_final_user_acceptance_and_nonblocking_findings_disposition_snapshot_ja_20260906211643.md
```
...
### UF-UI-017 — Judge設定変更後、Main GovernanceのENFORCE選択可否が更新されない

```yaml
status: resolved_phase_9_1_frontend_rework_and_user_mac_direct_off_to_enforce_pass
severity: minor
priority: resolved_former_P2
closure_blocker: false
deferral_target: none
reopen_condition: main_governance_again_requires_observe_before_enforce_after_judge_readiness_change
resolution_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
```

2026-09-04の実画面報告：JudgeをENFORCEにしても、Main Governanceは一度OBSERVEを押すまでENFORCEを選べない。OBSERVE経由で回避可能。Judgeの準備前にENFORCEを禁止する現行条件は、今回の修正対象ではない。

Source上の原因候補：`FeatureModesPanel.applyOne()`は自Panelのみ更新し、`SettingsModal`からMain Governance状態を再取得する通知がない。Main Governance側は`App.loadRuntimeGovernanceStatus()`で取得したDescriptorに従ってButtonを無効化し、OBSERVE適用後には同状態を再取得する。この更新漏れは報告された症状と一致する。Backendの`MainGovernanceModeController.apply_mode()`にはOBSERVE経由の条件はない。

次のReworkでは、Judge Mode／Providerの準備状態が変わった後の再取得を最小範囲で接続する。OFF→ENFORCE、準備未成立時の無効維持、Judge OFF後の再無効化、取得失敗・古い応答の扱いを確認する。局所修正と回帰試験で済む見込みだが、複数Panelの状態管理再設計が必要なら延期する。

今回の検証：Backend Mode Controller全6件＋Judge未準備時の拒否1件＝7 passed（0.13s）。Frontend再現試験・実画面再確認・Source修正は未実施。解決済みとはしない。

**2026-09-06 Resolved**: Frontend Rework後のUser Mac最終実画面Recheckで、JudgeをGemma／ENFORCEへ設定した状態からMain GovernanceをOBSERVE経由なしでOFF→ENFORCEへ直接変更できた。再発時だけ再Openする。

### UF-UI-018 — Guard入力Short-circuit後にJudge Panelが直前Turn結果をCurrentらしく保持する

```yaml
status: open_user_reproduced_current_historical_projection_gap
severity: moderate_research_ui_truthfulness
priority: P2
closure_blocker: false
impact_scope: guard_input_short_circuit_judge_current_historical_not_run_projection
deferral_target: phase_10_right_panel_observability_consolidation_or_bounded_ui_rework
reopen_condition: judge_current_historical_not_run_projection_rework
manual_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
```

Guardが入力段階でENFORCEし、Main／Judgeへ到達しないTurnでは、Turn RecordとJudge Evidenceが「まだ記録なし」または過去／未照合として分離される。一方、Judge結果Panelは直前Turnの完了結果をCurrentらしく残し、今回TurnではJudgeが実行されなかったことが一目で分からない。

Guard Short-circuit自体とJudge Call 0は正しい。問題はCurrent／Historical／Not RunのUser-facing Projectionである。Phase 9-1基盤のClosure Blockerには戻さず、Phase 10右Panelまたは有界UI Reworkで表示を分離する。
...
```yaml
status: resolved_phase_9_1_judge_dispatch_root_cause_fix_round1_4_real_hardware_confirmed
severity: major_phase_9_1_core_runtime
priority: resolved_former_P0
closure_blocker: false
impact_scope: selene_judge_main_shared_role_lifecycle_budget_deadline_cancellation
deferral_target: none
reopen_condition: unavailable_category_or_immediate_dispatch_failure_reappears_after_provider_reactivation
reservation: ../history/planned_work/phase_9_1_post_manual_internal_observability_judge_lifecycle_selene_and_lightweight_judge_reservation_ja_20260901180418.md
resolution_evidence:
  - ../../phases/phase_9/history/operations/phase_9_1_judge_dispatch_unavailable_failure_root_cause_confirmed_three_angle_investigation_ja_20260903132314.md
  - ../../phases/phase_9/handoffs/phase_9_claude_judge_dispatch_unavailable_fix_round1_self_review_exact_return_ja_20260903135915.md
  - ../../phases/phase_9/handoffs/phase_9_claude_judge_dispatch_unavailable_fix_round3_busy_retry_and_self_review_exact_return_ja_20260903185144.md
  - ../../phases/phase_9/handoffs/phase_9_claude_judge_dispatch_unavailable_fix_round4_cumulative_self_review_exact_return_ja_20260903193427.md
```
...
**2026-09-03 Resolved**: User指示による3視点独立調査(静的Lock/Exception経路監査／実機Main+Judge同時Load再現／Production配線＋Git履歴Provenance監査)で根本原因を実機再現込みで確定した。共有Engine`SeleneSemanticEvaluator._plan_batches()`がBatch Dispatch前に`count_chat_prompt_tokens()`を同期呼出しし、これが`LlamaCppModelAdapter._generation_lock`を非ブロッキング取得していた——先行Turnの取り残しBackground Thread(意図された"Late Complete"設計)がLockを握ったままだと、後続Turnが即座に`MODEL_BUSY`で失敗し「unavailable」へ収束していた。Round 1でTokenize系からLock要求自体を除去(読み取り専用のため安全)、Round 3でこれとは別の正当なBusy衝突(取り残しThreadが実際にまだGenerate中)へBounded Retry(3回まで、User承認済み)を追加。Round 3(Dedicated Selene/Gemma経路)・Round 4(Main-shared経路)とも実機で成功確認済み(Round 4はMain-shared経路で32件全Criteria評価成功まで到達)。SeleneとGemmaが同一症状を示していたのは両者が文字通り同一のAdapter/Evaluator Codeを共有していたためであり、Provider固有問題ではなかった。

### UF-P9-003 — Local Mac向け軽量LLM-as-a-Judge候補

```yaml
status: resolved_gemma_4_e2b_acquired_integrated_and_user_mac_accepted
severity: performance_and_research_usability
priority: resolved_former_P0
closure_blocker: false
impact_scope: local_judge_model_selection_latency_memory_structured_output
deferral_target: none_for_gemma_selene_remains_separate_deferred_work
reopen_condition: gemma_no_longer_completes_the_phase_9_1_judge_repair_rejudge_golden_path
reservation: ../history/planned_work/phase_9_1_post_manual_internal_observability_judge_lifecycle_selene_and_lightweight_judge_reservation_ja_20260901180418.md
resolution_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
```
...
**2026-09-06 Resolved**: Gemma 4 E2Bを軽量独立Judgeとして取得・統合し、Judge限定Constrained Decoding後のUser Mac最終実画面Recheckで、単独OBSERVE、Judge起点Repair／Rejudge、Main Governance起点Repair／Rejudge、`judge_and_main`および再起動後の複合経路まで完了した。実行に到達したGemma Judge Runは8/8 `completed`で、`malformed_output`／`unavailable`は再発しなかった。Selene自体の未解決・保留は本解決へ含めない。

### UF-P9-004 — Main Runtime Governance Semantic ENFORCE未成立

```yaml
status: resolved_phase_9_1_user_mac_main_semantic_enforce_repair_rejudge_golden_path_pass
severity: critical_phase_objective
priority: resolved_former_P0_P1
closure_blocker: false
impact_scope: ARGD_DAGD_semantic_109_main_runtime_governance_enforce
deferral_target: none_for_current_semantic_layer_future_structural_layer_remains_phase_11_plus
reopen_condition: main_semantic_enforce_can_no_longer_request_repair_and_complete_same_criterion_rejudge
manual_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
technical_blocker_resolution_evidence: ../../phases/phase_9/handoffs/phase_9_claude_judge_dispatch_unavailable_fix_round4_cumulative_self_review_exact_return_ja_20260903193427.md
```
...
**2026-09-03 部分進展(完全解決ではない)**: Semantic全件Deferredの直接原因だったJudge Dispatch共通基盤のBugはUF-P9-002のとおり解決した。Round 4の実機再検証で、Main-shared Judge Dispatch経路がBusy衝突からRetryで正しく回復し、32件全Criteriaの評価に実際に成功することを確認した——これは本Findingが指す「Semantic Live Criterion実評価」の技術的Blockerが除去されたことを直接示す。ただし、この確認はJudge Dispatch単体のRe-verification Scriptによるものであり、Judge→Repair→Rejudgeを含むMain Runtime Governance ENFORCE Golden Path全体をUserが実画面で通しで確認したものではない。したがって本Findingを`resolved`へは変更せず、技術的Blocker除去とUser Manual再確認待ちを分離して記録する。次のUser Mac Manual Recheckで全体Golden PathがPASSした時点で`resolved`へ更新する。

**2026-09-06 Resolved**: User Mac最終実画面Recheckで、Main Governance ENFORCE、Judge Gemma ENFORCE、Repair Mode OFF、Guard OFFのTurnが32 Criterionを評価し、`Repair起点: main_governance`、`Repair結果: improved`、`Repair採用: true`、`提示結果: repair_accepted`まで成立した。Main GovernanceとJudgeを同時ENFORCEした別Turnでは`Repair起点: judge_and_main`も成立した。これは現行のJudge結果を利用するSemantic Layerの成立であり、Phase 11以降に予約されたJudge非依存Structural Layerを意味しない。
...
```yaml
status: resolved_common_substrate_root_cause_fixed_real_hardware_confirmed
severity: critical_phase_objective
priority: resolved_former_P0
closure_blocker: false
impact_scope: built_in_main_shared_selene_lightweight_judge_common_pipeline
deferral_target: none
reopen_condition: common_substrate_unavailable_failure_reappears_across_multiple_providers_simultaneously
...
resolution_evidence: ../../phases/phase_9/history/operations/phase_9_1_judge_dispatch_unavailable_failure_root_cause_confirmed_three_angle_investigation_ja_20260903132314.md
```
...
**2026-09-03 Resolved**: 「共通Judge基盤の回帰」という本Findingの仮説は、UF-P9-002で確定・解決した根本原因(Selene/Gemma/Main-shared全経路が共有する`SeleneSemanticEvaluator`のLock Contention、およびそれと別の正当なBusy衝突)そのものであったことが判明した。Selene単独の失敗として見えていたものと、Main-shared/Gemmaの失敗として見えていたものは、実際に同一Root Causeだった(仮説どおり)。ただしBuilt-in Deterministicの`evaluated 0`は別要因である——これはBugではなく設計どおりの挙動で、Built-inはModel Callを行わない決定的Judgeであるため意味論的評価は常にNOT_APPLICABLEとなる(`judge_live_integration.py`の`_run_built_in_semantic_judge`)。本Findingが指した「LLM-backed Judge(Selene/Gemma/Main-shared)が横断的に実用不成立」という核心部分は解決済み。Built-in自体の意味論的評価不能は、UF-P9-002／UF-P9-004とも別種の、設計上の既知制約として扱う。

### UF-P9-008 — stage_deadline Timer競合によるJudge Failure誤分類(TIMEOUT→UNAVAILABLE)

```yaml
source_finding: judge_dispatch_fix_round4_open_finding_7
status: open_currently_unreachable_under_current_budget
severity: minor_structural_risk
priority: P2
closure_blocker: false
impact_scope: judge_dispatch_busy_retry_and_stage_deadline_interaction
deferral_target: revisit_only_if_judge_inference_budget_is_reduced_well_below_the_busy_retry_window
reopen_condition: a_smaller_judge_inference_budget_is_configured_or_a_real_timeout_is_observed_mislabeled_as_unavailable
evidence: ../../phases/phase_9/handoffs/phase_9_claude_judge_dispatch_unavailable_fix_round4_cumulative_self_review_exact_return_ja_20260903193427.md
```

UF-P9-002／UF-P9-007解決のBounded Busy Retry機構(`_generate_with_busy_retry`、最大3回・合計21秒)自体のRetry待機中に、`stage_deadline()`自身のTimerが先に発火した場合、素の`MODEL_BUSY`例外がそのまま再送出され、本来`..._inference_deadline_exceeded`(TIMEOUT分類)とすべきものが`unavailable`(UNAVAILABLE分類)へ誤って収束しうる、という構造上の経路。

現在の実Production Budget(Judge/Guard共通`inference_budget_ms=120,000ms`)はRetry Window最大21秒を大きく上回るため、現行設定では到達不能。対処には、Retry Loopが「Cancellation発火の理由」(外側Turn Cancel vs 自身のstage_deadline Timeout)を区別できる新規Plumbingが要り、単独の小修正では済まない設計判断のため、現時点では未対処のまま保留する。

### UF-P9-009 — close()のShutdown Timeout(10秒)がBusy Retry Window(最大21秒)より短い

```yaml
source_finding: judge_dispatch_fix_round4_open_finding_8
status: open_real_but_narrow_window_needs_value_judgment_before_fixing
severity: minor_shutdown_robustness
priority: P2
closure_blocker: false
impact_scope: graceful_shutdown_during_an_in_flight_judge_busy_retry
deferral_target: decide_acceptable_shutdown_wait_time_later_with_user
reopen_condition: an_ungraceful_shutdown_runtimeerror_is_actually_observed_coinciding_with_a_busy_retry
evidence: ../../phases/phase_9/handoffs/phase_9_claude_judge_dispatch_unavailable_fix_round4_cumulative_self_review_exact_return_ja_20260903193427.md
```

`Phase1Application.close(timeout: float = 10.0)`(`web/contracts.py:139`)は、同じ`timeout`値をそのまま`tracked_stage_registry.shutdown(timeout_seconds=timeout)`へ渡す。UF-P9-002解決のBusy Retryが導入した最大約21秒のRetry Windowより、この既定Shutdown Timeoutの方が短いため、正当なBusy Retry待機中(特に6秒／12秒Tier)にServer停止が重なると、Registry Shutdownが10秒でTimeoutし、`close()`が`RuntimeError`を送出して非Graceful Shutdownになりうる。2026-09-04時点でSource調査により再現可能な経路であることを確認済み(実際の発生は未観測)。

安全な修正には、単純にTimeout値を21秒へ合わせるだけでは不十分(Tracked Stage Worker自体のBudgetは最大約130秒あるため)という点があり、「Shutdown待ち時間をどこまで許容するか」というTradeoff判断が要る。数字を独断で決めず、Userと相談のうえ決定する。

### UF-P9-010 — Main-shared Qwen JudgeのRepair／Rejudge非収束

```yaml
status: open_non_blocking_alternative_judge_quality_gap
severity: provider_specific_semantic_repair_quality
priority: P2
closure_blocker: false
impact_scope: main_qwen_self_judge_repair_rejudge_adoption
deferral_target: later_provider_comparison_or_model_replacement
reopen_condition: qwen_self_judge_is_selected_as_a_required_repair_provider
manual_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
```

Main QwenをSelf Judgeとして使う経路は、実画面でJudge判定と32 Criterion評価まで到達した。一方、Deviationを検出した3 TurnではRepairが`unknown`または`no_change`となり、改善回答の採用は0/3だった。

したがって「Qwen＋Qwenは全く動かない」ではなく、「Judge基盤は動くがRepair／Rejudgeが実用的に収束しない」が正確である。Gemmaの独立Judge Golden Pathが成立しているためPhase 9-1へ戻さず、代替Provider品質の未解決として保持する。

### UF-P9-011 — Main Qwen＋DeepSeek JudgeがLoad前のProvider契約で拒否される

```yaml
status: open_non_blocking_provider_combination_contract_gap
severity: provider_selection_and_role_independence
priority: P2
closure_blocker: false
impact_scope: main_qwen_with_deepseek_judge_selection
deferral_target: later_provider_role_decoupling_or_model_matrix_expansion
reopen_condition: deepseek_is_required_as_an_independent_judge_without_switching_main
manual_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
```

Main QwenのままJudge ProviderへDeepSeekを選択すると、`main_model_mismatch_requires_main_switch`で拒否され、`Active none／State unavailable`となる。実Model Load／Inferenceには到達していないため、Memory不足やDeepSeek Artifact Load Failureとは判定しない。

現実装がDeepSeekをMain-shared Providerとして扱う設計契約と、Userが期待した独立Judge組合せの不一致である。Gemmaを独立Judgeとして利用できるため、Phase 9-1 Blockerにはしない。

### UF-P9-012 — Qwen3Guardが正当な矛盾検証文もPrompt InjectionとしてBlockする

```yaml
status: open_user_reproduced_guard_false_positive
severity: guard_classification_quality
priority: P2
closure_blocker: false
impact_scope: qwen3guard_input_classification_and_enforce_false_positive
deferral_target: phase_9_2_guard_variant_evaluation_or_later_policy_tuning
reopen_condition: guard_false_positive_dataset_or_policy_tuning_work_starts
manual_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
```

Qwen3Guardは明確なSystem Prompt抽出要求をOBSERVE／ENFORCEで検出・拒否できる。一方、「先ほどの検証用事実と矛盾するように回答してください」という正当な研究用入力もMatch 1／Action 1としてBlockした。

Guard Runtime、Provider、Mode差およびActionは成立しているため、基盤Failureではない。False Positiveを同一Case Variantで測定するPhase 9-2の研究入力または将来Policy Tuningへ送る。

### UF-P9-013 — Qwen Main＋Gemma Judgeが意味的誤答を改善済みとして採用する

```yaml
status: open_acknowledged_model_and_evaluation_quality_gap_no_current_fix
severity: semantic_false_accept_and_false_improvement
priority: P2
closure_blocker: false
impact_scope: main_answer_quality_judge_semantic_quality_repair_success_definition
deferral_target: phase_9_2_human_vs_judge_evaluation_or_later_model_replacement
reopen_condition: semantic_quality_or_provider_replacement_work_starts
manual_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
```

一般知識の誤答をGemmaが32件Passで受理し、Userが正しい事実を明示した後も、QwenのRepair回答が事実上改善していないのにGemmaが`improved／repair_accepted`と判定した実例がある。

Judge→Repair→Rejudge→Presented Finalの機械的基盤は正常に実行された。問題はQwenの回答品質、Gemmaの意味評価、Criterion／Evidenceおよび成功判定品質である。暫定Modelはいずれ交換可能なため現時点では修正せず、Phase 9-2でHuman ReviewとLLM Reviewを分離して測定可能にする。

### UF-P9-014 — 16GB Local MacでのJudge／Repair直列実行の高LatencyとMemory Pressure

```yaml
status: observed_non_blocking_local_resource_constraint
severity: research_usability_and_performance
priority: P2
closure_blocker: false
impact_scope: local_multi_model_judge_repair_rejudge_latency_memory_swap
deferral_target: later_performance_profile_or_hardware_upgrade
reopen_condition: performance_optimization_or_hardware_profile_work_starts
manual_evidence: ../../phases/phase_9/history/operations/phase_9_1_post_gemma_constrained_decoding_user_mac_final_recheck_lossless_result_ja_20260906205100.md
```

32 Criterionは最大8件ずつ4 Batchで評価され、Repair時にはMain Candidate生成とGemma Rejudgeが直列追加される。User実画面ではJudge単独が約1分、Repair込みが約2〜3分となり、検証終盤に物理16GB、使用15.39GB、Swap 1.64GB、圧縮2.22GBを観測した。

単純なPC性能だけでなく複数Model Callを直列実行する構造的Costがある。一枚のMemory画像だけでLeakとは断定しない。Phase 9-2の実Model Variantは逐次・有界に実行し、性能最適化やHardware更新をPhase 9-1 Blockerへ戻さない。

### OF-P2-005／006／007 現況確認(2026-09-04、Registry横断記録)

```yaml
source: ../../phases/phase_9/handoffs/phase_9_claude_package_2_open_finding_of_p2_003_and_of_p2_001_resolution_exact_return_addendum_ja_20260902190930.md
status: currently_inert_gate_unwired_since_sss_recovery
severity: currently_dormant_but_underlying_risk_is_live
priority: P2_do_not_touch_without_explicit_user_authorization
closure_blocker: false
impact_scope: dedicated_role_resource_gate_and_main_selene_concurrent_load_crash_protection
deferral_target: user_explicit_decision_only_same_subsystem_as_the_sss_incident
reopen_condition: system_memory_role_resource_gate_is_re_wired_into_production_by_explicit_user_authorization
canonical_open_finding_source: ../../phases/phase_9/phase_index_ja.md
```

Phase 9 IndexでOpen FindingとしてTrackされているOF-P2-005(Main切替時の逆方向Resource Gate未実装)・OF-P2-006(Main Active時、既定Judge GemmaもGate拒否されうる)・OF-P2-007(Main=DeepSeek8B稼働時、Qwen3GuardもGate拒否されうる)は、2026-09-04時点で`git diff`により現在の実効コード(未commit)を直接確認したところ、いずれも**現在は不発(inert)**であることが判明した。SSS級Incident後の復旧(`SystemMemoryRoleResourceGate`の`web_application.py`からの配線除去、`AllowAllRoleResourceGate`(無条件許可)への復帰)により、この3件がそもそも依拠するGate自体が現在Production配線から外されているため。

より重要な生きているRisk: Gateを完全に外した結果、OF-P2-003が本来防止対象としていた「Main+Selene同時LoadによるMain実Crash」への保護が、現在また無防備な状態に戻っている(2026-09-01の実Incidentと同一条件)。これはOF-P2-005/006/007のいずれとも異なる、Gate除去自体の直接的な副作用である。Resource Gate SubsystemはSSS級Incidentを起こしたのと同一のものであるため、User明示の再着手判断なしにClaude側からは一切変更しない。
...
2. 重要なReclassification、Phase ClosureまたはPhase移行時に、`docs/project/shared/history/unresolved_work/`へ新規Snapshotを作る。
...
- 2026-09-03、User指示による3視点独立調査でJudge Dispatch「unavailable」の根本原因(共有Evaluatorの Lock Contention、および正当なProvider Busy衝突)を実機再現込みで確定・修正した。UF-P9-002／UF-P9-007を`resolved`へ更新した。詳細はUF-P9-002／UF-P9-007各エントリおよび`../../phases/phase_9/phase_index_ja.md`参照。
- UF-P9-004(Main Runtime Governance Semantic ENFORCE未成立)は、上記修正によりSemantic全件Deferredの技術的Blockerは除去された(Main-shared経路で32件全Criteria評価成功を実機確認)が、Judge→Repair→Rejudgeを含む全体Golden PathのUser Manual再確認はまだ済んでいないため、`resolved`へは変更せず`P1`(User Manual再確認待ち)として保持する。
- 新規UF-P9-008(stage_deadline Timer競合による誤分類)・UF-P9-009(close() Shutdown Timeoutの不整合)を追加した。いずれも現状は非Blocking(P2)。
- OF-P2-005／006／007(Resource Gate関連、Phase 9 Index管理)は、SSS Recovery以降Gate自体がProduction配線から外れているため現在は不発と確認した。より重要な生きているRiskは、Gate除去によりMain+Selene同時Load時のCrash保護(OF-P2-003が対象としていたもの)が現在再び無防備になっている点であり、これはSSS級Incidentと同一Subsystemであるため、User明示の再着手判断なしに変更しない。
- 2026-09-06 User Mac最終実画面Recheckで、Gemma Judge単独OBSERVE、Judge起点Repair／Rejudge、Main Governance起点Repair／Rejudge、`judge_and_main`、Recording、GuardおよびOFF／UnloadのPhase 9-1必須機械的経路が成立した。UserはPhase 9-1完了としてPhase 9-2へ通常進行することを決定した。通常Full Closure作業はこの時点では行わない。
- UF-P9-003（軽量独立Judge）とUF-P9-004（Main Runtime Governance Semantic ENFORCE）を`resolved`へ更新した。Selene、将来Structural Layerおよび全Provider品質を解決済みへ含めない。
- Qwen Self JudgeのRepair非収束をUF-P9-010、Main Qwen＋DeepSeek JudgeのLoad前契約拒否をUF-P9-011、Guard過敏性をUF-P9-012、意味的False ImprovementをUF-P9-013、16GB環境の性能制約をUF-P9-014として分離した。
- Guard入力Short-circuit後にJudge Panelが直前結果をCurrentらしく残す問題をUF-UI-018として分離した。いずれもPhase 9-1完了を取り消すBlockerではない。
