...
updated_at: 2026-07-27 13:12:58 JST
owner: 設計統括者役
rag_default: true
```
...
- `docs/project/current/`と`docs/public/`の`_en`派生版は、ユーザーが作成時期を承認したDocumentation Refreshで作成する。Phase 1-exの今回の再構築では作成せず、Phase 1-ex後半で全対象を改めて判断する。
...
Phase 1-exの現在のCanonical／Public再構築は日本語正本だけを対象とする。Initial Commit前の最終Refreshで、Current／Publicの全`_en`派生版を同時作成するか、後続作業へ明示的に延期するかをユーザーが再判断する。英語版作成を自動的なPhase Gateへ変更してはならない。作成すると決定した場合は、対象文書すべてを日本語正本と同じ粒度で作り、部分的な抄訳を正式版として扱わない。
...
Initial Commit前には、実装後の状態に合わせてCurrent／Public／Setup／RAG／Public Demo／Git関連Docsを再編集し、Language Scope、Link、Identity、License、AllowlistおよびManifestを再検証する。`_en`派生版の作成有無はこの時点でユーザーが再判断し、その決定をInitial Commit対象Manifestへ明記する。
