...
ただし、規則を守ることと、判断を固定手順へ機械化することを混同しない。その時点の最高責任者役は、許可範囲、既存規則、作業目的、情報Loss、復元性、Risk、Cost、Context、Provider Capabilityおよび規則間Conflictを踏まえ、必要なRole、Document、Evidence、Handoff、Review、Test、Hard-code回避手段および停止地点を都度動的に判断する。

通常運転とAutomationで、この判断責任自体は変わらない。Automationが追加する差分は、ユーザー承認済み到達線とRole Authorityの内側で、最高責任者役がActionごとの追加確認なしにWork Unitを編成・連結できることである。人間専有判断または明示されたHuman Gateに該当しない事項を、Automationであることだけを理由に人間へ戻さない。
...
### 12.1 共通Document Authority／最高責任者役による動的Documentation判断
...
作業、担当、RoleまたはTaskごとに固定Artifact Packageを一律作成しない。その時点の最高責任者役が、Work Unit種別、Role／Task境界、State Transition、Mutation Risk、Review／Human Gate、Audit／Recovery要件、Provider Capability、情報Loss、復元性および運用Costを踏まえ、必要Document ClassとExact Pathを都度判断する。独立した機械的Resolverを前提にしない。
...
CoreへExact File名、件数または固定PackageをHard-codeしない。Project Bindingは許可Document Root／Classを与え、最高責任者役がWork Unit開始前または必要性判明時に対象をExact Pathへ固定する。この判断は既存Stableへの直書き、既存History Mutation、許可外Document Class、Authorized Root外またはExternal ActionのAuthorityを生成しない。

Phase Designerは、Automation中はユーザー承認済み到達線の内側で上位RoleのWork Unit指示に従う。通常運転中は、ユーザーが追加または変更したPhase要件を取り込み、Cross-Phase影響をProject Controller／Design GovernorへEscalateする。Context、安全性または実装規模上の必要がある場合、Phase別Implementerを新規配置できる。
