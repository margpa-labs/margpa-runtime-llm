...
本Revisionは、ユーザーの明示指示に基づき、可能な限りHard-codeを避け、どうしても必要な場合だけ管理された例外として許可する一般最上位規則を反映する。
...
Rule、Workflow、Path、Provider、Project、Phase、Task、Role Binding、Artifact名／件数、Threshold、Command、UI、Environmentその他の可変要素を、再利用されるCoreへ可能な限りHard-codeしてはならない。まず抽象化、Configuration、Manifest、Registry、Adapter、Profile、Schema、Runtime BindingまたはDynamic Resolutionを用いる。

Hard-codeは、技術的または論理的に不可避であり、かつ同等の抽象化手段では契約を維持できない場合に限って許可候補となる。採用時は、理由、代替案、代替不能性、Exact Scope、変更・Review方法、除去／Migration条件、TestおよびEvidenceを明示する。便宜、速度、現行Project／Providerへの最適化または「一時的」であることだけを理由にしない。

本規則はExact Runtime Valueを禁止するものではない。Project Manifest、Authorization Envelope、Role View、ConfigまたはFreeze Eventで実行時のExact Valueを解決することと、再利用されるCoreへ値を固定埋込みすることを分離する。
...
| Requirement Resolverが必要と判定した新規Work Unit Artifact | `CREATE_NEW` | `CREATE_NEW` | `CREATE_NEW` | `CREATE_NEW` | `CREATE_NEW` | `CREATE_NEW` | `REVIEW_ONLY` |
...
## 7. Dynamic Documentation Requirement Resolution

作業、担当、RoleまたはTaskごとに固定Packageを一律生成しない。必要Artifactは、Work Unit種別、Role／Task境界、State Transition、Mutation Risk、Review／Human Gate、Audit／Recovery要件およびProvider Capabilityから動的に解決する。
...
Role／Task間の移転Artifactには、論理的な送信元`from_role`と宛先`to_role`を必須とする。単一Role内の作業記録や機械的Evidenceへ、意味のないFrom／Toを強制しない。Read-only Roleの記録を別Roleが代行する場合は、論理的著者と移転関係を保持する。

CoreはArtifactのExact Name、件数または固定Packageを持たない。Project Bindingが許可されたDocument Root／Classを与え、Work Unit開始時にResolver結果をExact PathへFreezeする。Resolverは、既存Stable文書への直書き、既存History Mutation、許可外Document Class、Authorized Root外またはExternal ActionのAuthorityを生成しない。
...
- Role別実行権限。
- Role別Docs権限。
- Dynamic Documentation Requirement Resolution。
...
role_view:
  ...
  documentation_requirements: resolved_and_frozen
  human_gates: exact
...
- [ ] Work Unitの特性から必要Artifactだけを動的に解決している。
