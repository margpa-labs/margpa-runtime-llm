...
### 12.1 共通Document Authority／Task Artifact Contract
...
作業、担当、RoleまたはTaskごとに、既存Fileを再利用せず、新規のWork Unit／Task Index、Inbound Handoff、Outbound Statusおよび必要なReview／Acceptance Eventを作成する。Role間Handoff、Status、Review、RequestおよびAcknowledgementには、論理的な送信元`from_role`と宛先`to_role`を必須とする。
