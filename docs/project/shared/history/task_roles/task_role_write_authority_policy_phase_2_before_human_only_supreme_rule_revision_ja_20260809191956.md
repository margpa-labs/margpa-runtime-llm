...
- 更新日時: `2026-08-09 18:11:00 JST`
- Snapshot: `20260809181100`
...
### 10.1 Mutation Authorityは個別成立させる

全RoleのMutation Authorityは、[Research Asset Mutation Control](../operations/research_asset_mutation_control_ja.md)に従い、作業単位ごとに個別成立させる。
...
各Roleは、Mutation可能なTool Callの直前に次を確認する。
...
Project責任者は、絶対禁止事項、Docs規則、Authority規則、Mutation Control、Git／External Boundary、Backup、Evidenceおよび停止条件を他Roleと同様に遵守する。Project責任者であること、User Approvalを待っていること、進行責任を負うことまたはTask編成Capabilityを持つことを、運用ルールの免除根拠にしない。承認・確認待ちのため安全に停止することは、規則違反または責任放棄として扱わない。
...
自動化Pilotの正式Start Eventまでは現在Task名を変更しない。大規模Backup、Control TaskのReady宣言、後続User開始宣言およびAccepted Automation Profileが全て成立した直後に、Provider Capabilityが許す場合だけTask名を変更する。Task名変更不能または不一致時は停止する。
