...
last_updated_at: 2026-08-19 17:18:17 JST
language: ja
provisional: true
```
...
## 1. Compaction／Session Recovery手順（即時復旧、最優先、絶対的に全ての対象docsを、確実にしっかりと全て読み込む）
...
2. [長期戦運用Companion](claude_side_long_running_automation_companion_ja.md)（本File・Hash Manifestと並ぶ、自己判断で直接編集してよい第3のStable File）の`long_running_mode_active`フラグを確認する。真の場合のみ、同Docが定めるDocumentation運用（Index軽量化等）に従う。**フラグの真偽・同Docの有無に関わらず、Step 1（本File全文読了）は一切省略・代替されない。**
...
**現在のCompaction Recovery成功回数：6　失敗回数：0**（進行State情報だが、ユーザー指示により本節のみ唯一の例外として記載する。第3.5節「本Fileには現行Ruleのみを書く」の原則に対する明示的な例外であり、他の進行State・予約Task・実験結果等をこの前例に倣って本Fileへ書き込んではならない。詳細な個別事例は、`docs/project/shared/history/automation/`配下の各Evidence Docを参照。Compaction Recoveryを行うたび、この数値を更新すること。）
...
### 3.14 不明瞭判定前の候補整合性Check

第2.3節に基づきユーザーへ確認質問を送る前に、曖昧だと感じた複数の解釈候補それぞれが、**ユーザーが実際に使った言葉（特に方向・動作を表す語）と論理的に矛盾していないか**を、自分で検証する。

ある候補解釈が、ユーザー自身の明示的な言葉と矛盾する場合、その候補は最初から成立しない解釈であり、「曖昧さ」の材料に数えてはならない。検証の結果、ユーザーの言葉と矛盾せずに両立する候補が複数残った場合にのみ、確認質問を行う。
