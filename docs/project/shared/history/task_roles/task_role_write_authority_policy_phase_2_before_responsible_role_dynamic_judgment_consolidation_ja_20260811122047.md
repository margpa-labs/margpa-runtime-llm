...
### 12.1 共通Document Authority／Dynamic Documentation Requirement Contract
...
作業、担当、RoleまたはTaskごとに固定Artifact Packageを一律作成しない。Work Unit種別、Role／Task境界、State Transition、Mutation Risk、Review／Human Gate、Audit／Recovery要件およびProvider Capabilityから、必要Document Classだけを動的に解決する。
...
Role／Task間の移転Artifactにだけ、論理的な送信元`from_role`と宛先`to_role`を必須とする。CoreへExact File名、件数または固定PackageをHard-codeせず、Project Bindingが許可Document Root／Classを与え、Work Unit開始時にResolver結果をExact PathへFreezeする。
