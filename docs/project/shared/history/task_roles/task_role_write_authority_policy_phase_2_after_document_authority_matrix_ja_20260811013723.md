...
### 10.1 Mutation AuthorityはModeごとに成立させる

全RoleのMutation Authorityは、[Research Asset Mutation Control](../operations/research_asset_mutation_control_ja.md)と[Role Authority Matrix](role_authority_matrix_ja.md)に従い、通常運用とAutomation Modeを分離して成立させる。

通常運用では従来どおりMutationごとの明示許可を必要とする。Automation Modeでは、人間がAccepted化したEnvelope、Role Authority、Work Unit ScopeおよびControl State `ON`の交差内にある`AUTO` Actionに対し、Actionごとの再確認を要求しない。この事前授権がAutomationの実効権限である。

最上位規則、Envelope外、Role外、Authorized Root／Allowed Path外、`HUMAN_GATE`または`DENY`はAutomationでも置換できない。
...
意図、対象、Action、Root、Mutation有無、外部Access、委譲範囲または副作用のいずれかに1%でも不明点がある場合、必ずユーザーへ確認し、回答まで停止する。担当は、確認を省略することを自律性、効率、継続性または善意として正当化しない。
...
一方、Accepted Envelope、Role AuthorityおよびWork Unitに完全に含まれるActionは「意図不明」ではない。Automation `ON`の間は、通常運用のActionごとの再確認に戻さず、[Role Authority Matrix](role_authority_matrix_ja.md)に従って自律実行する。
...
### 12.1 Automation ModeのDocument Authority

Automation `OFF`の通常運用と、Accepted Envelopeが有効なAutomation `ON`を分離する。Automation `ON`では、[Role Authority Matrix](role_authority_matrix_ja.md)のDocument Authority Stateを必ずRole Viewへ展開する。
...
Control State `ON`中、EnvelopeとRole Viewの両方に含まれる`READ_AUTO`、`WRITE_STABLE_AUTO`および`APPEND_AUTO`は、Actionごとの再確認なしで実行できる。Stable更新に必要なBefore、After、Change RecordおよびIndex Snapshotは、同じDocument TransactionのAllowed Pathsとして事前に列挙する。

`READ_AUTO`はMutationを、`APPEND_AUTO`は既存Historyの編集を、`REVIEW_ONLY`は対象本文の修正を許可しない。Roleが自分のDocs Authorityを追加、拡張、兼務または代理しない。
