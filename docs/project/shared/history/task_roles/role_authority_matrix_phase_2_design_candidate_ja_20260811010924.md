...
## 11. Current Project Role Binding

このSectionはProject固有Bindingであり、Normative Coreではない。

| Current Role Name | Archetype Binding |
|---|---|
| プロジェクト責任者兼設計統括者役 | `project_controller` + `design_governor` |
| Phase 2設計担当者役 | `phase_designer` |
| Phase専用実装者役 | `implementer` |
| 対外Docs役 | `external_docs_editor` |

現時点のAutomation Pilotは未開始であり、本Bindingは未Accepted Envelopeを有効化しない。
