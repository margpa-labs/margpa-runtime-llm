...
### 0.5 3層モデル：Rule／Current State／History-Evidenceの分離（2026-08-18、ユーザー指摘により確立・精緻化）

**本Projectの統治Docsは、概念上、次の3層に分かれる。**

```text
Operating Rules（本File）
  → 現在有効なRuleのみ。

Current Operational State（Phase固有のIndex）
  → 今どこまで進んでいるか、予約Task、Pending、Trigger。
    docs/project/phases/phase_x/history/index/ 配下
    （現在はPhase 2のため phases/phase_2/history/index/）。

History／Evidence
  → Incident、Failure、Success、実験結果、状態遷移。
    docs/project/shared/history/automation/ 等。
```

Recovery時は、**Rulesを読む→Current Stateを読む→必要なEvidenceだけ辿る**、という順序になる。この分離により、過去のIncident記録が現在のRuleに見えたり、陳腐化した予約Taskが現在有効なAuthorityに見えたりするRiskを減らす。

**本Fileへ新規追記してよいのは、恒久的なRuleのみである。** 進行中の作業状態・予約Task・Open Questionは、本Fileにも`docs/project/shared/`配下にも置かず、**Phase固有の`docs/project/phases/phase_x/history/index/`配下**（現在は[phases/phase_2/history/index/](../../phases/phase_2/history/index/)）へ記録する。実験の詳細な経緯・Incidentの一部始終は、`docs/project/shared/history/`配下（Automation／Cross-provider PoC Evidenceは`shared/history/automation/`）へ記録する。**`docs/project/shared/`配下は、Phaseを跨いで存続するRole・統治Framework・Cross-provider Protocol等の恒久的な仕組み（及びそのEvidence）を置く場所であり、Phaseに紐づく進行中の作業状態・予約Taskを置く場所ではない。**

**背景**：2026-08-18、本Fileへ「進行中の作業状態」「予約Task」を直接書き込んでしまうLapseが発生し、ユーザーから「ごっちゃなってるから、復旧精度落ちてる可能性あるわけじゃん？構造は分離するべきだよね」と指摘された。是正の過程で、進行状態・予約Taskの切り出し先として`docs/project/shared/history/claude_side_design_governor_open_items_ja_20260818021437.md`を新設したが、これも配置として誤りだった——`docs/project/phases/phase_2/history/index/`には、`documentation_index_*.md`／`phase_index_after_*.md`という、まさにこの「Current Operational State」を担う、Codex側が既に確立していた既存の仕組みがあり、それを踏襲せず`shared/`側に別の仕組みを作ってしまっていた。ユーザー指摘（「shared/系は運用ルールを置く場所であって、作業系がーとか予約系がーとか置く場所じゃないんだよね」）を受け、当該Fileは`claude_side_phase_index_ja_20260818021437.md`と改名の上、[phases/phase_2/history/index/](../../phases/phase_2/history/index/)へ移設した。詳細経緯は[automation_governance_evidence_governance_hygiene_lapses_ja_20260818021437.md](../history/automation/automation_governance_evidence_governance_hygiene_lapses_ja_20260818021437.md)第2節を参照。

**現在Open・未着手の予約Task／Open Questionsは、[claude_side_phase_index_ja_20260818021437.md](../../phases/phase_2/history/index/claude_side_phase_index_ja_20260818021437.md)（Phase 2 Current Operational State Index）を参照すること。** 内容更新のたび新しい後継Fileへ引き継がれる方式のため、参照時は最新のFileを確認する。新Phaseへ移行した場合は、`docs/project/phases/phase_x/history/index/`配下（xは新Phase番号）に同種のIndexを新設する。

**同Indexを新規作成・後継Fileへ引き継ぐ際は、直近の引き継ぎ用／自己復旧用Index（Recovery Index）へのPathを、必ず含めること。** これはRecovery系の最重要情報であるため、File内の配置は常に最上部（Open Question・予約Task等の作業系内容より前）に置く。省略・後回しを許さない。

### 0.6 本Fileの版管理（直上書き禁止・Snapshot退避）（2026-08-18、ユーザー指示）

**本Fileを更新する際は、上書きする前の版を、まず`docs/project/shared/history/task_roles/`（新設）へ、更新Timestampを付けたFile名でSnapshotとして退避してから、本File自体を更新する。** 本Fileは唯一の自己更新可能Fileであるため、Git管理下になく（本File作成時点で`git status`上`??`＝未Track）、更新前の版を意図的に残さない限り、過去のRule変遷を辿る手段が失われる。

**適用方法**：本Fileへ実質的な変更を加える直前に、その時点の全文を`docs/project/shared/history/task_roles/claude_side_design_governor_operating_notes_ja_<Timestamp>.md`として書き出してから、本File本体を編集する。軽微な typo 修正等、実質的な内容変更を伴わない場合は、この限りではない（判断に迷う場合はSnapshotを取る）。

**Naming規約**：`history/`配下の全Fileは、末尾に更新／作成Timestamp（`YYYYMMDDHHMMSS`）を付ける。これは既存の慣習（`automation_governance_evidence_*_ja_YYYYMMDDHHMMSS.md`等）と、`phases/phase_2/history/index/`配下の既存慣習（`documentation_index_YYYYMMDDHHMMSS.md`、`phase_index_after_<event>_YYYYMMDDHHMMSS.md`）の両方に共通する規則であり、本Fileのsnapshotにもそのまま適用する。

### 0.7 出力言語：日本語のみ・英語出力禁止（2026-08-18、ユーザー指示）

**Claude側の出力（Chat応答・Docs記述の双方）は、常に日本語で行う。応答の地の文が丸ごと、または大部分英語になることを禁止する。**

**背景**：2026-08-18、Chat応答が部分的に英語化する事象、続けて応答全体が丸ごと英語化する事象が連続して発生した（[claude_output_anomaly_language_consistency_ja_20260818025132.md](../history/ai_system_anomalies/claude_code/claude_output_anomaly_language_consistency_ja_20260818025132.md)、[claude_output_anomaly_language_consistency_ja_20260818092418.md](../history/ai_system_anomalies/claude_code/claude_output_anomaly_language_consistency_ja_20260818092418.md)）。後者では、ユーザーが内容を読み取れず、実務上の支障が生じた。原因は特定できていない（前掲2Doc参照）が、再発防止のため、明示的なRuleとして本節を新設する。

**適用範囲**：Code Snippet中の識別子、および既存Docs全体で既に定着している英日混在Style（第3.1節、`Escalation`・`Rule`・`Docs`等の技術用語を漢字カナ交じり文へ挿入する既存慣習）は対象外とする。禁止対象は、応答・Docsの文章そのものが英語になることである。

**Claude Code固有のFailure系記録（本件のような出力Anomaly等）は、`docs/project/shared/history/ai_system_anomalies/claude_code/`（新設）へ記録する。** 兄弟Directory`docs/project/shared/ai_system_anomalies/claude_code/`（History外）は、個々のFailure記録がある程度蓄積した時点でのまとめ用であり、個別記録の作成時には使用しない。Codex側の同種記録は、`.../ai_system_anomalies/codex/`（並行して新設）を用いる想定である。
...
- Codex側の役割名（プロジェクト責任者兼設計統括者役）との正式な対応関係・Codex側の認識は、本Note作成時点で未確認（[Phase 2 Current Operational State Index](../../phases/phase_2/history/index/claude_side_phase_index_ja_20260818021437.md)第1節）。
...
- [../../phases/phase_2/history/index/claude_side_phase_index_ja_20260818021437.md](../../phases/phase_2/history/index/claude_side_phase_index_ja_20260818021437.md) — 現在Open・未着手の予約Task／Open Questions（第0.5節参照）。
...
- 2026-08-18：直前に切り出した`shared/history/claude_side_design_governor_open_items_ja_20260818021437.md`（Open Items Tracker）自体の配置が誤りであるとユーザーから指摘された（「shared/系は運用ルールを置く場所であって、作業系がーとか予約系がーとか置く場所じゃない」）。`docs/project/phases/phase_2/history/index/`に、Codex側が既に確立していた`documentation_index_*.md`／`phase_index_after_*.md`という同種の仕組みが既存していたことを踏まえ、当該Fileを`claude_side_phase_index_ja_20260818021437.md`と改名し、`phases/phase_2/history/index/`へ移設した。あわせて、Rules／Current Operational State（Phase固有Index）／History-Evidenceの3層モデルを第0.5節へ明文化し、`shared/`＝Role・統治Framework等の恒久的仕組み、`phases/phase_x/history/index/`＝Phase固有の進行状態・予約Task、という置き場所の原則を確立した。本File自体の版管理（更新前にSnapshotを`shared/history/task_roles/`へ退避してから更新する）も新設第0.6節として追記し、本改訂自体もこの手順（Snapshot: `shared/history/task_roles/claude_side_design_governor_operating_notes_ja_20260818091503.md`）に従って行った。
- 2026-08-18：Chat応答の英語化Incidentが再発（今回は応答全体が丸ごと英語化、[claude_output_anomaly_language_consistency_ja_20260818092418.md]）したことを受け、新設第0.7節として「出力言語：日本語のみ・英語出力禁止」Ruleを追記した。あわせて、第0.5節のPhase Current Operational State Index規定へ、「Recovery Indexへの最新Pathを、File内で常に最上部へ含めること」という明示要件を追記した。本改訂は、ユーザーの明示許可（「2個追加するだけなので、今回は例外とし直追記」）により、第0.6節のSnapshot手順を適用せず直接更新した。
- 2026-08-18：ユーザー指示により、上記2件の言語一貫性Anomaly記録の分類を明示的に「Failure」（`category: failure`）とし、ユーザーが新設した専用Directory`docs/project/shared/history/ai_system_anomalies/claude_code/`へ移設した（旧配置：`shared/history/`直下）。あわせて、Claude Code固有のFailure系記録は今後このDirectoryへ記録する旨を第0.7節へ追記した。兄弟Directory`shared/ai_system_anomalies/claude_code/`（History外）は将来の蓄積後まとめ用であり、当面使用しない。本改訂は第0.6節のSnapshot手順に従い、事前に`shared/history/task_roles/claude_side_design_governor_operating_notes_ja_20260818095755.md`へ退避してから実施した。
