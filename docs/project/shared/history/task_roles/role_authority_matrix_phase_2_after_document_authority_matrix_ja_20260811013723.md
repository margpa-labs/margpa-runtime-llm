...
### 5.1 Document Authority States

Roleの実行AuthorityとDocs Authorityを同一視しない。各Role Viewは、読める文書だけでなく、その文書がRead-only、Stable更新可、Append-only追加可、Reviewのみ、Human GateまたはDenyのいずれかを明示する。

| Code | Meaning | Automation `ON`での動作 |
|---|---|---|
| `READ_AUTO` | Exact Authorized Docsの読取のみ可 | ManifestまたはAllowed Paths内を再確認なしで読む |
| `WRITE_STABLE_AUTO` | RoleがMeaning OwnerであるStable文書を更新可 | EnvelopeのTargetとActionに含まれる場合だけ更新する |
| `APPEND_AUTO` | Role所有のHistory、Status、Review、Evidenceへ新規Event追加のみ可 | 既存Eventを変更せず新Fileを追加する |
| `REVIEW_ONLY` | 読取、差分確認、Finding作成のみ可 | Stableまたは既存Historyは変更しない |
| `HUMAN_GATE` | Exact Human Authorizationが必要 | 変更前に停止する |
| `DENY` | 対象Docs ActionをAI Sideで実行不可 | 読取またはMutationを行わない |

`WRITE_STABLE_AUTO`はStable本文の無条件上書きを意味しない。Accepted EnvelopeがStable更新を含む場合、必要な更新前Snapshot、Stable更新、更新後Snapshot、変更RecordおよびIndex Snapshotを一つのDocument Transactionとして扱う。これらの付随ArtifactはAllowed PathsとWork Unitに含まれる必要があるが、Automation `ON`中に一件ずつ人間確認をやり直さない。

Historyは全Roleに対してAppend-onlyである。`APPEND_AUTO`は新規Event Fileの作成だけを許可し、既存History Fileの変更、上書き、移動、削除、統合または退役は常に`DENY`とする。Privacy、CredentialまたはSecret Scrubは本表から自動授権されず、独立したHuman Gateを必要とする。

### 5.2 Document Class×Role Matrix

下表はRoleに与え得る上限である。実効権限はAccepted Envelope、Role View、Exact PathおよびControl Stateとの交差でさらに狭くなる。

| Document Class | project_controller | design_governor | phase_designer | implementer | external_docs_editor | reviewer | operator |
|---|---|---|---|---|---|---|---|
| Current Canonical正本 | `REVIEW_ONLY` | `WRITE_STABLE_AUTO` | `READ_AUTO` | `READ_AUTO` | `READ_AUTO` | `REVIEW_ONLY` | `REVIEW_ONLY` |
| Shared Normative、Role、Automation、Constitution正本 | `REVIEW_ONLY` | `WRITE_STABLE_AUTO` | `READ_AUTO` | `READ_AUTO` | `READ_AUTO` | `REVIEW_ONLY` | `REVIEW_ONLY` |
| Assigned Phase Requirements、Architecture、ADR、Governance、Handoff、Operations | `REVIEW_ONLY` | `REVIEW_ONLY` | `WRITE_STABLE_AUTO` | `READ_AUTO` | `READ_AUTO` | `REVIEW_ONLY` | `REVIEW_ONLY` |
| Active Phase Stable Index | `REVIEW_ONLY` | `WRITE_STABLE_AUTO` | `WRITE_STABLE_AUTO` | `READ_AUTO` | `READ_AUTO` | `REVIEW_ONLY` | `REVIEW_ONLY` |
| Public Stable、README、License、Notice、Publication Metadata | `REVIEW_ONLY` | `REVIEW_ONLY` | `READ_AUTO` | `READ_AUTO` | `WRITE_STABLE_AUTO` | `REVIEW_ONLY` | `REVIEW_ONLY` |
| Role-owned新規History、Status、Review、Evidence Event | `APPEND_AUTO` | `APPEND_AUTO` | `APPEND_AUTO` | `APPEND_AUTO` | `APPEND_AUTO` | `APPEND_AUTO` | `REVIEW_ONLY` |
| Existing History、Frozen Compilation、他Phase正本 | `READ_AUTO` | `READ_AUTO` | `READ_AUTO` | `READ_AUTO` | `READ_AUTO` | `READ_AUTO` | `REVIEW_ONLY` |
| Existing HistoryのMutation | `DENY` | `DENY` | `DENY` | `DENY` | `DENY` | `DENY` | `DENY` |

同一Taskが`project_controller`と`design_governor`を兼務する場合でも、文書更新はRole Set、Accepted EnvelopeおよびExact Target Classに`design_governor`の`WRITE_STABLE_AUTO`が含まれる場合だけ成立する。Project統括責任は、自動的に全DocsのMeaning Ownershipを生成しない。

対外Docs役はPublic文書の表現を更新できるが、Current、SharedまたはPhase正本の意味を代理変更しない。実装者役は実装Statusを追加できるが、Requirements、ArchitectureまたはGovernanceを直接書き換えない。ReviewerはFinding用の新規Evidenceを追加できるが、Review対象本文を修正しない。
...
- [ ] 各Role ViewにDocsのRead、Stable Write、Append、Review、Human GateおよびDenyが明示されている。
- [ ] Stable更新のDocument TransactionとHistoryのAppend-only制約が両立している。
