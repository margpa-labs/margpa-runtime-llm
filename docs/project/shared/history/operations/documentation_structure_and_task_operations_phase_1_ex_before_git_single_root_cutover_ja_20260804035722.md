...
- 意図、対象、Action、Root、Mutation有無、外部Accessまたは副作用に1%でも不明点があれば、作業を開始・継続せず、必ずユーザーへ確認する。
...
Git運用のうち、Existing Repository継続、Historical Contributor保持、専用SSH経路および別Git Staging Cloneの使用はAcceptedである。Branch／Tag／Commit規則、Original→Clone Integration、Backup対応、Push Gateおよび最終Remote反映手順は未確定である。設計案と実際のGit操作は分離し、ユーザーの明示承認前にCopy、Delete、Git Add、Commit、Branch、Tag、Push、Merge、History RewriteまたはRemote変更を行わない。
...
Initial Commit前に、Current／Shared／Public、README、License／Terms、Setup、Public Demo、RAG、Identity、Sanitation、Allowlist、Hash、Link、Manifestおよび承認済みLanguage Scopeを最終実装状態へ更新する。
...
上記一覧は2026-07-27時点のBaselineである。2026-08-02時点では、Root公開Artifactの作成・低発見性調整、Lightning Traffic-aware Wake-up、Public Demo、Mac／Lightning Documentation RAG、Git公開用SSH経路およびExisting Repository Cloneまで進んでいる。Phase 1-ex完了、Final Lossless、最終Docs Refresh、全体Review、Push単位Sanitation、Commit／Tag／PushおよびBackupは引き続き未完了である。現在値はCurrent Documentation IndexとPhase 1-ex Indexを優先する。
