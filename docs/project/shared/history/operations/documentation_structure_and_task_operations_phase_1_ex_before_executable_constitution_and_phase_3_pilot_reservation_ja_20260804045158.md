...
`constitution/`はStableな統合憲法書、Manifest、Capability Contract、Provider AdapterおよびTemplateを持つSelf-contained Package候補である。`shared/history/constitution/`は変更前後SnapshotとVersion Migration EvidenceをAppend-onlyで保持する。
