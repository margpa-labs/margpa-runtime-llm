...
```text
READ_AUTO          : Exact Authorized Docsの読取のみ
WRITE_STABLE_AUTO  : Meaning OwnerによるStable更新
APPEND_AUTO        : Role所有の新規History／Status／Evidence追加のみ
REVIEW_ONLY        : Review／Finding作成のみ
HUMAN_GATE         : Exact Human Authorization後だけ実行
DENY               : 実行不可
```

詳細なDocument Class×Roleの上限表は[Role Authority Matrix](../task_roles/role_authority_matrix_ja.md)を正本とする。Phase固有のRole Viewは、正本Matrixを書き換えず、Accepted EnvelopeとExact Pathにより狭いViewだけを作る。

Stable更新のAuthorityが成立するWork Unitでは、更新前Snapshot、Stable更新、更新後Snapshot、Change RecordおよびIndex Snapshotを一つのDocument Transactionとする。Automation `ON`でも付随Pathを暗黙に追加せず、EnvelopeまたはManifestに事前列挙する。
...
Human-defined Supreme Rules
  ...
  → Work Unit Role View／Exact Paths
  → Documentation Rules
  → Ordinary Operational Defaults
```

Write Scopeが不明な場合、他担当領域へ黙って書き込まない。Automation `ON`中に、Role Viewが`READ_AUTO`としたDocsを書込可と解釈しない。また、Accepted Envelope内の`WRITE_STABLE_AUTO`または`APPEND_AUTO`を通常運用のActionごとの再確認へ戻さない。
