...
- 通常作業を直接`main`へCommit／Pushしない。
- 作業Branchは最新の`origin/main`から作成する。
...
- Phase 1-ex完了Tagは、Phase 1-exが未完了のため現時点では作成しない。
...
## 10. Publication Sanitation Gate

Push前には[GitHub Publication Sanitation Policy](git_publication_sanitation_policy_ja.md)に従う。

最低限、`git status`、Staged Diff、Outgoing Commit Range／Tree／Metadata、Identity、Secret、個人情報、不要物、License、Attribution、Ignore Rule、Test、Link、Manifest、Push対象SHAおよびRemote Branchを確認する。
...
Git Staging Clone             : Existing Git History、Remote連携、Commit／PR用
```

この構成では、GitHub更新のたびにSource→Git Stagingの同期、全Path／Content照合、Sanitationおよび差分Reviewが必要になる。したがって安全だが、恒久必須ではなく、同期漏れRiskと運用Costがある。

### 12.2 現時点の決定

- Phase 1-exの残作業中は、既に検証済みの二重Rootを維持する。
- `margpa-runtime-llm_git_staging`は、Phase 1-ex中のGit操作とRemote HistoryのLocal Working Copyとして保持する。
- OriginalまたはGit Stagingの削除、Rename、入替、`.git`移植または役割変更を本書だけで実行しない。

### 12.3 単一Git Rootへの将来移行

永久にGit Staging Cloneを別Rootとして使い続ける必要はない。Phase 1-ex Final Backup後、Phase 2開始前を第一候補として、Git管理された一つのWorking Rootへ統合できる。

```text
1. Original／Git Staging両方のBackupとSHA-512を取得
2. Git Staging main＝origin/main、Clean、git fsck PASSを確認
3. 公開対象FileのPath／Content一致を再検証
4. 今後のCanonical Pathをユーザーが選択
5. Git Clone全体を単一Working Rootとして採用
6. .venv、models Link、Local Secret、Runtime Stateを非追跡で再構築
7. Mac Runtime／Full Test／Lightning Deployment境界を再検証
8. 旧Rootは即削除せず、ユーザー判断までBackup／退避
9. Docs、Task、IDEおよびScriptのPath参照を更新
10. ユーザー承認後にCutoverを確定
```

`.git` DirectoryだけをOriginalへ手動Copyする方法は採用しない。既存Git Clone全体を基準にCutoverする方が、Index、Config、Remote、Hooks、Object DatabaseおよびWorktree整合性を保ちやすい。
...
- `.git`だけの手動CopyによるRoot統合
- Original／Git Stagingの無承認削除、Renameまたは入替
