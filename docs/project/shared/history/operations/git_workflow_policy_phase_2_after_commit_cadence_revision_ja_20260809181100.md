...
Commit／Pushは原則として、復元可能な大きな区切り、Subphaseの主要Milestone、Phase完了またはRisk上必要なCheckpointで行う。各Task往復、小さなDocs追記または軽微な状態更新ごとに反復しない。

```text
Docs History／Recoveryで中間状態を保持
  → 有界MilestoneまたはPhase Gateで差分を統合Review
  → Userが当該Commit／Pushを明示承認
  → Sanitation／Test／Scope確認
  → Commit／Push
```

このCadenceはCommit回数を減らす運用上の既定であり、長期間の巨大差分、復元困難な変更またはRiskの高いMutationを無保護で蓄積する理由にしない。事故、重大変更、Context交代またはRecovery Riskがある場合は途中Checkpointを提案する。Automation Profileが`phase`または`project`でも、Commit／PushのUser Explicit Authorizationは自動付与されない。
