...
- 必要Artifactは、その時点の最高責任者役が、Work Unit種別、Role／Task境界、State Transition、Mutation Risk、Review／Human Gate、Audit／Recovery要件、Provider Capability、情報Loss、復元性、CostおよびContextから都度動的に判断する。
...
ArtifactのExact Name、件数または固定Packageを共通CoreへHard-codeしない。許可Document Root／ClassはProject Bindingで与え、各Work Unitの必要ArtifactとExact Pathは、その時点の最高責任者役が必要性を判断して固定する。独立した機械的Resolverを前提にせず、判断責任は通常運転とAutomationで共通とする。この判断は既存Stableへの直書き、既存History Mutation、許可外Document ClassまたはAuthorized Root外へのAuthorityを生成しない。
