...
Finding分類は[Transition Blocker／Escalation／Closure Contract](transition_blocker_escalation_and_closure_contract_ja.md)を適用する。`Unresolved`、Evidence保持、過去Failureまたは将来の昇格条件だけではBlockerにならない。Current Transitionへの直接必要性、現在未解決および放置時のAcceptance／安全性／完全性／可逆性／Evidence Integrity／Authority影響で`HOLD／NONE`を決め、解決Authorityとは別にResolution Routeを決める。

次のFindingがCurrent Phase Acceptanceへ直接関係し、未解決のまま進むとAcceptanceまたは安全境界を破壊する場合は、解決責任者の有無に関係なくPhase TransitionをHoldする。担当Roleが解決可能ならUserへ返さず、完了後に再判定する。
...
設計統括者役が妥当性をReview済み
要求、Acceptance Scope、重大Riskまたは明示済みHuman Gateを変更する延期なら、ユーザーが明示的に受け入れ済み
```

条件を満たさないFindingは、LabelにかかわらずPhase完了前に担当Roleが解決する。Current Transitionへ影響せず、既に将来のResearch／Phaseへ割り当てられ、Owner、TriggerおよびVerificationが明確な`DEFERRED_EVIDENCE`は、最高責任者役が記録して非活性化し、Userへ延期判断を繰り返し要求しない。

未解決事項はTransition ImpactとResolution Routeへ分離する。
...
担当Roleが自Authority内で閉じられるStable整合、Evidence整理、ReviewおよびTestをUser Blockerへ付け替えない。一方、Current Transitionへ必須のRole-owned Workは完了まで`HOLD`し、次工程設計へ移送しない。

## 5. Phase完了順序

Phase Final Check後、最高責任者役は自分で`GO／ADJUST／STOP`を提案し、Technical Blocker、Role-owned Work、Deferred Evidence、ValidationおよびHuman-only Actionを分離したClosure Recommendationを作る。担当RoleのAuthority内で閉じられる作業を残したままUserへ「どうしますか」と返さない。
...
全Subphase実装／Review完了
  ...
  → 現Phase完了＋次Phase READYの同一SnapshotをGit運用に従いCommit／Tag／Push／GitHub更新
  → Local／origin／Remote一致確認
  → 次Phaseは別途定義された開始Gate成立後に開始
```

GitとBackupの正確な先後関係はAccepted Git Workflowに従う。ただし、現Phaseの確定状態と次PhaseのREADY入口を同一のPhase境界Snapshotに含め、Backup、Commit、Tag、Pushおよび公開Evidenceへ関連付ける。`次Phase READY`を次Phase開始、Mutation許可またはAutomation `ON`と混同しない。
...
原則として、Backup、Git Commit／Tag、GitHub更新およびRelease Evidenceは、現Phase完了と次Phase READYが同時に確認されたPhase境界単位で対応付ける。ただし、各ActionはそれぞれのAuthorityとユーザー明示承認を必要とし、Phase完了または次Phase READYだけを理由に自動Commit、Tag、Push、Uploadまたは公開を行わない。

Phase間の大規模Checkpoint BackupはPhase Backupを代替しない。Phase BackupはFinal Check、User Acceptance、現Phase完了、次Phase READY、Continuity RefreshおよびReconstruction Validationに対応する確定Snapshotとする。
...
- 自Authority内のStable整合、Evidence整理、Preflight、推奨判定および次工程設計を自分で閉じる。
- Current Blocker、Role-owned Work、Deferred EvidenceおよびUser Gateを分離し、Human-onlyでない判断をUserへ返さない。
...
- Backupを実施し、完了を明示する。
- 要求、Acceptance Scopeまたは重大Riskを変更するFinding延期、Git／GitHub反映、公開およびPhase移行のHuman-only Gateを最終承認する。
...
- Blocker Eligibilityを満たす未解決Blocker、またはCurrent Transitionへ直接影響する条件不備のDeferred Item
- User Acceptance未完了
...
- [Transition Blocker／Escalation／Closure Contract](transition_blocker_escalation_and_closure_contract_ja.md)
...
## 12. Pre-pilot Broad Backup／Restore Evidence

未知のFailure Modeを含む初回Automation Pilot等では、ユーザー判断によりProject Backupより広い範囲のBackupを開始前Gateにできる。ただし、広域Backupの実施、保存先、暗号化、Cloud Account、Credentialおよび復元操作はユーザー担当であり、AI／Task／ToolはAuthorized Root外へRead／Writeして証明しようとしてはならない。

Backup Evidenceでは、次を区別する。
...
ユーザーが必要と判断した場合の確認候補は、暗号化状態、完了時刻、対象範囲、除外対象、保持先、復元手順およびSample Restoreである。これらは自動的な必須項目ではなく、RiskとBackup方式に応じてユーザーが採否を決める。

初回Pilot前の大規模Backup完了報告はPilot開始Gateの一つであるが、それだけでAutomationを開始しない。Accepted Profile／Envelope、Control TaskのREADY宣言および後続のユーザーStart宣言を別に必要とする。
