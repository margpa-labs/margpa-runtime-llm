...
Existing RepositoryのHistory継承、Source→Target統合、Draft PR／Merge、作業Branch退役および単一Git Root CutoverはAcceptedである。現在は`margpa-runtime-llm`がGit Working Treeと開発内容の単一Canonical Rootであり、旧Git Staging CloneはBackup後にユーザー判断で退役済みである。

Canonical Rootでの`git status`、`git diff`、`git log`、`git fsck`、Remote Read-only Query、Hash取得およびTestは、対象と停止条件が固定できる場合、通常GPT＋ユーザー手動操作へRoutingできる。

Commit、Push、Merge、Tag、Release、Branch削除、History変更、Sanitation Candidateの処置、Remote／Visibility変更およびCanonical Rootの構造変更は設計統括者役へ戻し、対象ごとのユーザ明示承認を必須とする。Source実装はAccepted Handoffに従って実装者役へ渡す。

Risk-based Git Workflowにより、小規模で決定論的なDocs／Metadata変更はDirect `main`を候補にできる。新機能、複数Layer、大規模、高RiskまたはPhase統合はBranch／Draft PRを原則とする。どちらも自動実行せず、ユーザ承認とPublication Gateを省略しない。
...
## 14. Future Experiment Reservation

Codex Desktopの独立Task作成、命名、Pin、Prompt送信、進捗確認およびReviewをDocument-driven運用へ接続する構想は、[Experimental Document-driven Codex Task Orchestration](experimental_document_driven_codex_task_orchestration_ja.md)へ分離して予約する。

現時点では実験未開始であり、ユーザーの明示指示なしに新Taskを作成しない。Phase 2を最初の正式Pilotとし、[Phase 2 Subphase／Task Orchestration Preplan](phase_2_subphase_and_task_orchestration_preplan_ja.md)により2-A～2-Fの分割、Role間Gate、実装者Task更新、Resource LimitおよびAuthority逸脱対策を予約する。ただし、Phase 1-ex完了、User Acceptance、Backupおよびユーザー承認なしにPhase 2へ移行しない。

## 15. Resource Limit／Task Renewal／Authority Deviation

Codex利用可能量、Credit、QuotaまたはService Limitによる途中停止を運用前提とする。停止時は未完了状態をAccepted／Completeと表記せず、最後の確認済み状態、Open Finding、次の最小ActionおよびRecovery Pathを固定する。別Model、別Account、追加課金または大量の代替Taskへ自動切替しない。

Phase専用実装者Taskを基本案とし、Context Limit、古いHandoffの混同、Status不備、繰り返し失敗、Authority逸脱またはPhase変更で安全な継続性が下がる場合にTask更新を検討する。更新前に旧TaskのWriteを止め、Status、Mutation Inventory、Test、Open Finding、Git／External StateおよびRecovery Pathを固定する。

設計統括者役を含むすべてのRole／Task／Agentは、権限外または運用ルール外Actionを取りうる。Role名、上位責任、長期の成功実績またはTool Permissionは遵守の保証ではない。Mutation前のAuthority解決、完全な変更列挙、Cross-role Review、Stop GateおよびRecovery Evidenceを必須とする。逸脱または疑いを検出したら、自動修復や無許可Rollbackを行わず即時停止する。
