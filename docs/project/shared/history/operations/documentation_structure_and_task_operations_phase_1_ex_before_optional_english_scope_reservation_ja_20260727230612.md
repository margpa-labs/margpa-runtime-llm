...
英語版を当該Refreshの作成対象へ含めた場合は、JA／EN Pairの同等性を確認できないCurrent／Public文書を更新完了または公開可能と判定しない。Conflict時の正本は常に日本語版である。

Phase 1-exの現在のCanonical／Public再構築では日本語正本だけを作る。`_en`派生版はPhase 1-ex後半のInitial Commit前Refreshでユーザーが全対象について再判断する。英語版を作成すると決定した場合は、概要・抄訳ではなく日本語正本と同じ粒度で一式を作る。英語版を自動的な公開Gateへ追加してはならない。
...
Initial Commit前に、Current／Public、README、License／Terms、Setup、Public Demo、RAG、Identity、Sanitation、Allowlist、Hash、Link、Manifestおよび承認済みLanguage Scopeを最終実装状態へ更新する。
