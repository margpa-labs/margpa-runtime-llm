...
Docs Authorityは共通Role契約の独立Dimensionとし、Role Viewは`READ／CREATE_NEW／APPEND_NEW／EXISTING_WRITE_USER_EXPLICIT／REVIEW_ONLY／DENY`を明示する。既存Stable文書への直書きは、Modeを問わず、ユーザーがExact TargetとActionを明示した場合だけ成立する。Work Unitごとの新規Index／Handoff／Status／ReviewおよびFrom／ToもMode共通である。
...
Document Authority     : mode-invariant common matrix corrected／role view draft-2 pending review
