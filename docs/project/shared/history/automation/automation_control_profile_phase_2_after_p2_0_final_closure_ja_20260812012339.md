...
status: validated_bounded_unit_profile
normative: true
language: ja
...
`OFF`はRule無効化ではない。`EMERGENCY_STOP`後にAI側がCleanup、RollbackまたはState再開を自己承認しない。P2-0-WU-003のProvider Grammar違反後は`PAUSED／CAPABILITY_CONTRACT_REDESIGN`へ移行し、P2-0-WU-004の独立再試験、User Final AcceptanceおよびP2-0 Final Acceptanceを経て、現在は`OFF／P2-0 CLOSED／PHASE 2-A READY／BACKUP・START PENDING`である。
...
## 11. Validated P2-0 Bounded-unit Profile

```yaml
level: bounded_unit
control_state: OFF_P2_0_WORK_UNIT_CLOSED_FINAL_CONFIRMATION_PENDING
completed_work_unit: P2-0-WU-002
adjust_required_work_unit: P2-0-WU-003
accepted_work_unit: P2-0-WU-004
...
task_creation:
  allowed: false_until_next_exact_package_and_required_gate
  maximum_active: 1
  maximum_replacement: 0
existing_task:
  action: no_further_action_authorized
read:
  manifest: p2-0-documentation-capability-manifest-001_exact-1
...
document_projection:
  ...
  exact_paths: frozen_and_consumed
  narrower_work_unit_constraint: one_exact_create_completed
mutation:
  filesystem: completed_one_exact_create
  existing_file: deny
...
result: capability_semantics_conformance_pass
controller_review: accepted
user_final_acceptance: accepted_closed
promotion: denied_until_additional_evidence
status: validated_bounded_unit_p2_0_closed_phase_2_a_ready
```

Project固有Path、Provider ToolおよびTask内部IDは本Profile Coreへ埋めず、Start Event時のManifest／Adapter／Evidenceで解決する。

このP2-0固有Profileでの`readable`は、共通Matrixの`READ`を今回のWork Unitへ狭めた結果であり、Automation専用の別Docs権限を定義するものではない。WU-004合格とP2-0 Final Acceptanceは`workflow／phase／project`への昇格を生成しない。一方、上位Levelの未検証事項はClosed済みP2-0または次の`bounded_unit`設計を止めるBlockerとしてユーザーへ再判断させない。
