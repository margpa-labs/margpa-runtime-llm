...
status: current_pre_activation_gate_reconciled
normative: false
language: ja
...
ARMED
  Design、Envelope、Preflight、Git Checkpoint、BackupおよびREADY Evidenceを満たし、Two-key Activation待ち。
...
Control StateとAutomation Levelは独立である。`project + OFF`は自動実行を行わず、`bounded_unit + ON`は一つのAccepted Work Unit内だけを実行できる。現在のStateは`OFF`である。
...
## 8. BackupとRecovery Evidence

初回Pilot開始前のPC広域Backupは、ユーザーが実施し完了を明示する。AI側はProject Root外を読み、検査し、Copyし、Archiveし、Uploadし、Permission変更しまたはBackupを自動作成しない。

将来、ユーザーが必要と判断する場合は、次をBackup Evidence候補とする。現時点でAIが確認する立て付けではない。
...
`Backup exists`と`Restore verified`を混同しない。
...
特定Provider、Project、Repository、Absolute Path、Phase、Task名、CommandまたはUIをNormative CoreへHard-codeしない。CoreはCapability、Authority、Evidence、State、Scope、Stop、RecoveryおよびHuman Gateで定義し、Project固有値はManifest、Provider固有操作はAdapter、Work Unit固有値はEnvelopeへ分離する。
...
Design Review                    : gate reconciliation complete／final validation pending
