...
本境界は現在確認済みの最上位規則群の一つである。最上位規則群は将来のEvidenceにより追加され得るが、Automation Profile、RoleまたはProviderが自動で追加・削除・上書きできない。
...
Incident、Authority逸脱、Folder Boundary違反、未観測Mutation、Cost急増、Provider不整合、Recovery不成立またはHuman Gate省略を検出した場合は即時停止し、`manual`またはSafe Profileへ降格する。自動Rollbackで追加Mutationしない。
