...
`OFF`はRule無効化ではない。`EMERGENCY_STOP`後にAI側がCleanup、RollbackまたはState再開を自己承認しない。Phase 2-0初回Unit終了後の現在Control Stateは`PAUSED／ROLE_AUTHORITY_DESIGN`である。
...
Automation `ON`では、Accepted Envelope、共通Role AuthorityおよびRole Viewに含まれる`ROLE_ALLOWED` Actionだけを連結実行する。既存Stable文書への直書きは、ユーザーがそのExact TargetとActionをAccepted Completion Lineへ明記した場合だけ成立する。必要Artifactは通常運転と同じDynamic Documentation Requirement Resolverで解決し、Automation専用の固定Packageを作らない。Role／Task間の移転ArtifactにだけFrom／Toを要求し、HistoryはAppend-onlyを維持する。
...
ユーザーが最上位規則として明示した一般Hard-code禁止に従い、Normative Coreは特定Provider、Project、Repository、Phase、Task、Role Binding、Artifact名／件数、Threshold、Command、UI、EnvironmentまたはAbsolute Pathを可能な限りHard-codeしない。
...
Hard-codeは、技術的または論理的に不可避で、同等の抽象化手段では契約を維持できない場合に限る。理由、代替案、代替不能性、Exact Scope、変更・Review方法、除去／Migration条件、TestおよびEvidenceを記録する。便宜、速度、現行環境への最適化または「一時的」は十分な理由ではない。

Project Manifest、Authorization Envelope、Role View、ConfigまたはFreeze EventでExact Runtime Valueを解決することは、再利用されるCoreへの固定埋込みと区別する。

### 8.0.1 Dynamic Documentation Requirement Resolver

Work Unit Artifactは固定名または固定件数で定義しない。ResolverはWork Unit種別、Role／Task境界、State Transition、Mutation Risk、Review／Human Gate、Audit／Recovery要件およびProvider Capabilityから、Index、Handoff、Status、Review／Acceptance、EvidenceまたはArtifact不要を選択する。

Project Bindingは作成可能なDocument Root／Classを与え、Work Unit開始時にResolver結果をExact PathへFreezeする。Resolverは既存Stableへの直書き、既存History Mutation、許可外Document Class、Authorized Root外またはExternal Actionを許可しない。
...
role_authority_matrix: role_authority_matrix_pending_user_review
role_view: p2-0-role-view-phase-designer-001_pending_acceptance
...
document_projection:
  ...
  common_state: READ
  exact_paths: p2-0-read-manifest-001_exact_entries_only
