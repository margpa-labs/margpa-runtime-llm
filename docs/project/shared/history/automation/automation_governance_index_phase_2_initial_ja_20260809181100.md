...
3. プロジェクト責任者兼設計統括者役による「準備OK。いつでも開始出来ます。」の明示。
4. その後のユーザーによる「ok。では開始する。」の明示。
5. 対象Automation Profile、Authorized Root、Allowed Actions、Task上限、期限およびHuman Gateの最終一致。
