...
Automation LevelとControl Stateを分離する。Control Stateは`OFF／ARMED／ON／PAUSED／EMERGENCY_STOP`を基本状態とし、現在は`OFF`である。詳細とREADY Evidenceは[Pre-pilot Automation Governance Baseline](pre_pilot_governance_baseline_ja.md)に従う。
