...
Docs Authorityは共通Role契約の独立Dimensionとし、Role Viewは`READ／CREATE_NEW／APPEND_NEW／EXISTING_WRITE_USER_EXPLICIT／REVIEW_ONLY／DENY`を明示する。既存Stable文書への直書きは、Modeを問わず、ユーザーがExact TargetとActionを明示した場合だけ成立する。必要Artifactは、その時点の最高責任者役がMode共通のDocs／運用規則に沿って都度判断し、Role／Task間の移転ArtifactにだけFrom／Toを要求する。
...
### 8.1 最高責任者役の動的Documentation判断

Automationは判断まで機械的に固定しない。必要Artifact、統合可否、Exact PathおよびFrom／Toは、その時点の最高責任者役が、共通Role／Docs Authority、情報Loss、State、Risk、Review、Recovery、Cost、ContextおよびProvider Capabilityから都度判断する。

Project Bindingは許可Document Root／Classを与え、最高責任者役が必要な対象を固定する。固定Packageまたは別のResolver Subsystemを設けず、この判断によって既存Stableへの直書き、既存History Mutation、許可外Document ClassまたはAuthorized Root外へのAuthorityを生成しない。
...
Document Authority     : mode-invariant common matrix corrected／responsible-role judgment consolidated／role view draft pending review
