...
### 3.1 Documentation Capabilityは独立Dimension

Filesystem Mutationが許可されても、全DocsへのWrite Authorityは発生しない。各Role Viewは[Role Authority Matrix](../task_roles/role_authority_matrix_ja.md)のDocument Authorityから、Read、Stable Write、Append-only Add、Review-only、Human GateおよびDenyをExact PathまたはDocument Classごとに投影する。
...
Automation `ON`では、Accepted EnvelopeとRole Viewに列挙されたDocs Actionだけを自律実行する。Stable更新に付随するBefore／After Snapshot、Change RecordおよびIndex Snapshotも事前にAllowed Pathsへ含める。既存HistoryのMutationはAutomation LevelにかかわらずDenyとする。
...
Automation LevelとEnvelopeだけでは実行主体の権限を確定しない。[Role Authority Matrix](../task_roles/role_authority_matrix_ja.md)を用い、Role上限とAccepted Envelopeの交差をEffective Authorityとする。

Control State `ON`で、両者に含まれる`AUTO` Actionは、通常運用のActionごとの確認を再要求せず自律実行する。`REVIEW`、`HUMAN_GATE`、`DENY`、Envelope外またはRole外は自動拡張しない。
...
documentation:
  ...
  human_gated: []
  denied:
    - all_document_mutation
    - existing_history_mutation
mutation:
  filesystem: deny
  git: deny
  external: deny
