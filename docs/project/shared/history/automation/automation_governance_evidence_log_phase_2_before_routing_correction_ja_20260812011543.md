...
27. Current Blockerは、現在Transitionへの直接必要性、現在未解決、委譲Authority内で解決不能、放置時の安全・完全性・可逆性・Evidence Integrity・Authority破壊の全条件を満たす場合だけ認定する。
28. 未解決事項を`CURRENT_BLOCKER／RESPONSIBLE_ROLE_OWNED_WORK／DEFERRED_EVIDENCE／USER_GATE`へ分類し、未解決であることだけをUser Escalation理由にしない。
29. Accepted／Closedその他の確定済みHistorical Outcomeを、明示TriggerなしにActive Governance StateまたはCurrent Blockerへ戻さない。
30. Human Decision Burdenを有限Resourceとして扱い、各Roleが委譲範囲内で決められる事項をUserへMicro-escalateしない。
...
| Blocker Eligibility／Responsibility-first Escalation／Human Decision Budget／Closure Contract | Authority Roles／Escalation／Human Gate／Resource Budget／Task Lifecycle／Review |
