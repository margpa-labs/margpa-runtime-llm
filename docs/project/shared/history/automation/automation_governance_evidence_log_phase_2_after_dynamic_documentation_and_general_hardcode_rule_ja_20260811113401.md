...
16. Role／Docs／Dynamic Documentation Requirement規則は通常運転とAutomationで共通化し、Automation側には承認済み到達線と連結実行差分だけを持たせる。
17. Work Unitの特性から必要Artifactだけを動的に解決し、Role／Task間の移転ArtifactへFrom／Toを記録する。固定Packageを作らない。
...
| Dynamic Documentation Requirement Resolution | Document Source of Truth／Resource Budget／Context Control／Portability |
