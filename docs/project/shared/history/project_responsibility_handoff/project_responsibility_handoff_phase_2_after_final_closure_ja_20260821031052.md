...
source_role: プロジェクト責任者兼設計統括者役
active_gate: phase_3_ready_not_started
```
...
設計統括者役のRecoveryを置き換えない。当面は一つの現在Taskが`プロジェクト責任者兼設計統括者役`としてProject ResponsibilityとDesign Governanceを兼務する。プロジェクト責任者責務はProject全体の編成とGateを扱い、設計統括責務は技術設計、要件、Architecture、Canonical Docs、Phase設計とTechnical Reviewの継続性を担う。両Folder／Stable／History／Recoveryは分離したまま相互参照し、兼務を理由に一方へ吸収しない。
...
10. [Active Phase Index](../../phases/phase_2/phase_index_ja.md)
11. [Public Roadmap](../../../public/roadmap_ja.md)
12. [Automation Governance Index](../automation/automation_governance_index_ja.md)
13. [Automation Control Profile](../automation/automation_control_profile_ja.md)
...
Phase 1-exのFinal Docs、Lossless、Recovery、Test、Sanitation、Backup、Git Postflightは合格し、ユーザーはPhase 1-ex完了とPhase 2開始を明示した。現在はPhase 2-0 Automation Pilotの設計Review待ちである。Phase 2開始だけを根拠に、Task作成、Pilot実行または元来のPhase 2機能実装を行わない。

## 8. Current Phase 2-0 Gate

```text
Phase 1-ex               : COMPLETE／ACCEPTED
Phase 2                  : STARTED
Active Subphase          : Phase 2-0 Automation Pilot Design
Pilot Design Package     : DRAFT COMPLETE／REVIEW PENDING
Authorization Envelope  : DRAFT／NOT ACCEPTED
Independent Task Created: NO
Pilot Executed           : NO
Phase 2 Feature Work     : NOT STARTED
Next User Gate           : design review／envelope acceptance／explicit task creation request
```

Project責任者役は、設計統括者役が作成したPilot Design PackageをCross-Phase不変条件、User Authority、Cost、RecoveryおよびStopの観点で調整する。技術的内容のMeaning Ownerを奪わず、Accepted Envelopeが存在するように装わない。

Phase 1-ex ClosureとPhase 2 Pilot設計から得た知見は[Automation／Governance Evidence Log](../automation/automation_governance_evidence_log_ja.md)へ累積記録する。Phase 2-0では、その知見をDocs-only Recovery、Authority Acknowledgement、段階的Automation Profile、明示State Machine、Semantic Freshness Review、Backup／Dual Consent Start Gateおよび安全停止へ反映する。

## 9. Phase 2 Final Closure／Phase 3 READY Successor Gate

本節は旧Phase 2-0 Gateを履歴として保持したまま、現行Project Gateを定義する。

```text
Phase 2                  : COMPLETE／ACCEPTED／CLOSED
Final Technical Blocker : NONE
User Backup             : REPORTED COMPLETE／PRIVATE ASSET OUTSIDE AI GATE
Phase 3 Design          : ACCEPTED／FROZEN
Phase 3                 : READY／NOT STARTED
Automation              : OFF
Git Boundary            : PHASE 2 CLOSED＋PHASE 3 READYを同一Snapshotで反映
Tag／Release            : NOT REQUESTED
```

プロジェクト責任者兼設計統括者役は、Phase 3開始時にPreflightとRecovery Freshnessを確認し、`ARMED`を明示してからユーザーの開始宣言を待つ。Phase 3 Ready化、Phase 2完了または本Handoffだけを根拠にTask、Claude実行、MutationまたはAutomationを開始しない。
