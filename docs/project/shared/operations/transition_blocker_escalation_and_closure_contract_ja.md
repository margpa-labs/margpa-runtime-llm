...
## 3. Transition ImpactとResolution Routeの分離

Findingは一つのLabelへ潰さず、次の2軸で最高責任者役がルーティングする。

```yaml
finding_route:
  transition_impact: HOLD | NONE
  resolution_route: ROLE_OWNED_CURRENT | ROLE_OWNED_NEXT | HIGHER_ROLE | USER_GATE | EXTERNAL_WAIT | DEFERRED_EVIDENCE
  responsible_role: exact
  completion_point: exact
  evidence: exact_reference
```

`transition_impact: HOLD`は、次の3条件を全て満たす場合に成立する。

1. 現在要求されているTransitionの成立条件へ直接必要である。
2. 現時点で未解決である。
3. 未解決のままTransitionすると、Acceptance、安全性、完全性、可逆性、Evidence IntegrityまたはAuthority Boundaryを破壊する。

解決可能なRoleが存在しても、上記3条件を満たす間は`HOLD`を維持する。解決責任者がいることはUser Escalationを不要にするが、未完了の必須条件を満たしたことにはしない。

`CURRENT_BLOCKER`は、最高責任者役のClosure出力で、`HOLD`が残り、かつ現在のAccepted Authority内で解消できず、Human Gate、External Waitまたは別の解消条件を待つ状態を表す。Role-owned Current Workは完了までTransitionを保持するが、Userへ返すBlockerではなく、Closure報告前に担当Roleが閉じる。

重大に見えること、過去に失敗したこと、未検証であること、Evidenceとして残っていること、または将来必要になることだけでは`HOLD`または`CURRENT_BLOCKER`にならない。

## 4. Mandatory Routing

### 4.1 `ROLE_OWNED_CURRENT`

現在のTransitionへ必要であり、担当Roleまたは上位Roleが委譲済みAuthority内で解決できる。`HOLD`を維持したまま、調査、設計、修正、Stable整合、Evidence整理、TestまたはReviewを完了し、再判定する。未着手または作業中であることをUser Blockerへ付け替えない。

### 4.2 `ROLE_OWNED_NEXT`

現在のTransition成立条件ではなく、次のAccepted Work Stageで担当Roleが処理する。Current Transition Impactは`NONE`であり、次作業として引き継ぐ。現在の必須条件を「次で行う」と移送するために使用してはならない。

### 4.3 `HIGHER_ROLE`

担当RoleのAuthority外だが、直属上位Roleまたは最高責任者役の委譲済みAuthority内で解決できる。Userへ直接送らず、責任階層に沿って上げる。

### 4.4 `DEFERRED_EVIDENCE`

将来の研究、昇格、移植性、性能、Provider差、長期安定性または別Phaseの検証対象である。Current Transition Impactは`NONE`とし、定義済みTriggerが到来するまでActive Governance Stateへ戻さない。

### 4.5 `USER_GATE`

人間にしか決定、実施または受容できない事項である。Technical Blockerと混在させず、Exact User Actionとして別表示する。

### 4.6 `EXTERNAL_WAIT`

担当RoleまたはUserの判断では直ちに解消できない外部状態待ちである。Current Transitionへ必須なら`HOLD`、必須でなければ`NONE`とし、再確認TriggerとOwnerを記録する。
...
Finding／Uncertainty
  ...
  -> Current Transitionへ直接必要か
     ...
     -> YES: 自Roleの責任・権限内で解消可能か
        -> YES: HOLDを維持し、調査・設計・修正・検証してEvidence化
        -> NO: 直属上位Roleの委譲範囲内か
           -> YES: Responsibility Escalation
           -> NO: Human-only／External Wait Eligibilityを判定
```

安全停止は「Userへ聞く」を自動的に意味しない。停止後も、担当Roleと上位Roleは自Authority内の解消責任を保持する。

最高責任者役は、複数のFindingを統合し、Current Transition Impact、解決責任者、Resolution Route、再開条件、Human Gate適格性およびClosure Recommendationを確定する。分類表をUserへ渡して選ばせるのではなく、自Authority内でルーティングを完了する。
...
Human Escalationは、次の判定原則のいずれかを満たす場合に成立する。

- 決定、同意、実施またはRisk受容が、全AI Roleへ委譲されたAuthorityを超える。
- Userへ明示的に予約されたGateまたは責任である。
- Accepted Objective、Scope、Authority、Root、不可逆性または外部責任を変更する。
- 最高責任者役が自Authority内の調査、設計、検証および上位Role Routingを尽くしても、安全かつ可逆的に決定できない。

次は非網羅的な代表例であり、閉じたAllowlistではない。未列挙事象も上記判定原則により最高責任者役が動的にルーティングする。
...
1. 新しいCurrent Transitionが、そのHistorical Outcome、失敗Dimensionまたは未解決前提へ新たに依存する。
2. 既存TransitionのDependency、利用Scope、前提またはCompletion Lineが変化し、Historical Outcomeとの関係が変わる。
3. Current Transitionの真偽を変える新しいEvidence。
4. Current Transitionが依存するArtifactのIntegrity Mismatch。
5. 現在適用される上位規則との新たなConflict。
6. Userによる明示的な再Open指示。
...
Transition Holds:
  NONE | exact unmet current prerequisite and owner
...
Closure報告前に、担当Roleの権限内で閉じられる作業を残さない。Transition Holdが残る場合は`GO`を出さず、Ownerと解消地点を確定する。`Deferred Evidence`を列挙してもよいが、Current Transition Impactが`NONE`なら判断要求へ変換しない。
...
2. 新しいTransitionが過去Failureへ依存する場合、Evidenceの新旧に関係なく再評価する。
3. Stable正本が古い場合、Authorityを持つControllerが自分で整合してから報告する。
4. 次Phaseの設計作業を前PhaseのBlockerとしてUserへ返さない。
5. 現在必須のRole-owned Workを次工程へ移送せず、完了までTransition Holdを維持する。
6. Multi-provider、長時間安定性または上位Automation研究を、現在の有界Transitionへ混入させない。
7. Current Transitionに必要でAuthority外の重大問題は、正しく上位RoleまたはUserへEscalateする。
8. Human Gateを減らすためにAuthority、Root、Externalまたは不可逆境界を推測拡張しない。
9. 未列挙のHuman-only事象を、閉じた列挙から漏らさず判定原則でルーティングする。
10. Closure出力が推奨判定、Blocker、Deferred、ValidationおよびHuman-only Actionを分離する。
