...
updated_at: 2026-08-02 14:58:25 JST
owner: 設計統括者役
rag_default: true
```
...
Local ProjectにGit Metadataがない状態から更新するため、Remote History未確認のまま現在Project Rootを初期化して上書きPushしない。ユーザーが明示指定した別作業場所へExisting RepositoryをCloneし、Historyを確認してから現在Projectの公開対象差分を反映する方式を優先する。Clone、Copy、同期、Commit、RemoteおよびPushは未実行である。
...
`CITATION.cff`はGitHubのMachine-readable Citation MetadataとCitation UIを有効化するため削除推奨とした。README／TERMSから参照を除去済みであり、LocalおよびGitHub Default Branchからの削除はユーザーが手動で行う。Task側は削除していない。
...
2. Basic認証Previewと分離した最小Public Demo基盤実装。Public専用Rate／Budget／Cost制限は`off` Hookとし、Public Documentation RAGを強制無効にする。Repository Review後のLightning設定と匿名公開有効化はユーザーが行う。
