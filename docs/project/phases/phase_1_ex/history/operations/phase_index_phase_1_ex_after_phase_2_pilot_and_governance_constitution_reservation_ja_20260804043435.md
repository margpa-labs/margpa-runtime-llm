...
現設計統括者役はProject責任者としてProject全体、Cross-Phase不変条件、Task編成、Handoff、ReviewおよびRecoveryを統括する。ユーザーの最終Decision Authorityは維持する。

Pilotは一つの有界なWork Unitから始める。必要Taskの作成、Task名設定、Authority設定、Task間Handoff／Status／Follow-up／ReviewをAccepted Envelope内で連結し、安定後だけ複数Unit、Subphase、Phase完了単位へ拡張する。Project完了単位は長期目標であり、自動移行しない。
...
Agent／Tool本格実装前に、絶対禁止、Docs、Authority、Mutation、Handoff、Review、Recovery、Backup、Git、Costおよび停止規則をProvider-neutralな統合憲法書へLossless再編する。`docs/project/shared/constitution/`を他Projectへ配置して同等の開発体制を再構築でき、Codex Desktop／Claude Codeの双方へAdapter可能なPackageを目標とする。
