...
## 2.20 Future Document-driven Codex Task Orchestration Experiment
...
設計統括者役がPhase Index／Handoffを作成した後、必要な独立Codex Taskを直接作成・命名・Pinし、初回PromptでReading Order、Authority、禁止事項およびHandoff Pathを送り、進捗・Review・Follow-upまで管理するDocument-driven Orchestrationを将来実験として予約した。

Phase 2設計担当者役を最初の正式Pilotとする。ただし実験は未開始であり、Phase 1-exの完了、User Acceptance、Backupおよびユーザーの明示的なTask作成指示が揃うまで新Taskを作成しない。独立Taskは継続Role、Sub-agentは一時的な探索／Test／Triage等へ使い分け、Write-heavy作業は原則直列化する。

Phase 2は`2-A Phase Contract／Conversation Domain Foundation`、`2-B Conversation Persistence／Lifecycle Services`、`2-C Conversation Application UX`、`2-D Configuration Control Surface／Research Developer Mode`、`2-E Runtime Composition Switchboard／Documentation RAG Follow-up`および`2-F Cross-environment Acceptance／Phase Closure`の6 Subphaseへ分割する。Phase 3～Phase 9への横展開はPhase 2 Pilotの成功後に都度判断し、Phase 10以降は現時点の対象外とする。

Phase 2設計担当者役とPhase 2実装者役は専用の独立Taskを基本案とする。実装者TaskはPhase内で安全に継続できる間は再利用し、Context Limit、Resource Limit、Authority逸脱、Status不備または繰り返し失敗時は、旧TaskのWriteを停止しRecovery Evidenceを固定してから更新する。

Codex利用可能量またはCreditでどのTaskも途中停止しうる。また、設計統括者役を含む全Role／全Taskが権限外または運用ルール外Actionを取りうる。未完了状態をCompleteと表記せず、Role名やTool Permissionに依存せず、Authorization Envelope、Mutation Inventory、Cross-role Review、Stop GateおよびRecovery Evidenceを使用する。

ユーザーは要件変更、Backup、Destructive Action、External Service／Secret、Commit／Push／公開、User AcceptanceおよびPhase移行のAuthorityを保持する。本予約は完全自律化またはUser Gate省略を許可しない。

## 2.21 Phase Final Check／Backup Gate共通化

- [Phase Completion Review／Backup Gate](../../shared/operations/phase_completion_review_and_backup_gate_ja.md)

各Phaseは、個別SubphaseのAcceptedまたは実装完了だけで完了としない。Phase全体のFinal Checkを必須とし、Findingは原則として当該Phase内で全て解決する。`non-blocker`のLabelだけで延期せず、共通Gateの条件、設計統括Reviewおよびユーザーの明示承認が揃った場合だけ例外的に次Phaseへ送る。

設計統括者役は、Continuity RefreshとReconstruction Validationの完了後、ユーザーが自発的にBackupを取得する予定であっても、必ず「Phase Backupを取得してください」と明示する。Backup、Git Commit／Tag、GitHub更新およびRelease Evidenceは原則Phase完了単位で対応付ける。大規模変更、復元困難な変更、破壊的操作、Git／公開変更、Cloud再構築または長期作業では、Phase途中でも規模／RiskベースのBackup Checkpointを勧告する。

ユーザーがAccepted Orchestration Envelope内で「じゃ、あとよろしく」と委任した場合、次の確認時までに少なくとも1つの有界なWork Unitを完了、Review待ちまたはEvidence付きの安全なPauseへ到達させることを運用目標とする。これは時間保証、User Gateの代行、権限越境または未検証完了を許可しない。

## 2.22 Git Delta Inventory／統合直前Backup
...
Read-only Delta Inventoryは、OriginalとCloneを変更せずに完了した工程としてAcceptedとした。ただし、これは統合実行のAcceptedではない。5 UPDATE Candidate、767 ADD Candidate、41 Clone-only Candidate、16,809 Ignore Artifactおよび1 External Model Symlinkを検出し、Source→Target Integration Manifestで最終分類する。

Git Staging Cloneの`.git/`とExisting HistoryをGit履歴正本とし、現在のOriginal Project Working Treeを次Commit内容正本とする。統合はRoot-to-Rootとし、Clone Root下へOriginal Root Directory自体を入れ子でCopyしない。無分類上書き、一括削除同期、`models`、`.venv/`、Cache、Secretおよび未承認PathのCopyを禁止する。

ユーザーはOriginalとGit Staging Cloneの統合直前ZIP Backupを取得した。両ZIPのFile SHA-512、CRC、Staging `.git/`収録、Original `models` Symlink非追跡および両Directoryの決定論的Tree SHA-512を検証済みである。本Checkpointは実統合前Backup Gateを満たすが、Phase 1-ex完了Backupを代替しない。

長時間Orchestrationは、例外、不確実性、Authority待ち、Test FailureまたはResource Limitで無理に進めず、Evidence付きの安全な中断を優先することを再確認した。

## 2.23 Command-only Request誤実行Incident／絶対禁止追加
...
ユーザーが外部`other/`のPermission変更Commandの提示を求めたにもかかわらず、設計統括者役がCommandを実行した。その後の短い否定を「再帰的に実行する」と誤解し、ACLとGroup／Others Permissionを再帰的に変更した。これはCommand提示と実行許可を混同したCritical Governance Deviationである。

「コマンドをくれ」「僕がやる」「キミがやるんではなく」等は`output_only`であり、実行を絶対禁止する。実行は当該ターンでユーザーが対象とActionを明示して「キミが実行して」と依頼した場合だけ可能とする。Approval UI、Sandbox Escalation、Tool Permission、Role Authorityまたは過去の許可は、このSemantic Authorizationを代替しない。

誤実行後のPermissionはユーザーの追加指示なしにRollbackしない。本Incident後の外部Mutationは停止している。

## 2.24 明示確認原則／Workspace外周境界／自動化実験分離
...
「良かれ」「推測」「話の流れ」「過去の許可」「効率」「安全化」またはRoleの責務から、存在しないAuthorityを生成することを絶対禁止とする。意図、対象、Action、Root、Mutation有無、外部Accessまたは副作用に1%でも不明点があれば、必ずユーザーへ確認し、回答まで停止する。

本Project作業の外周境界は`MARGPA-RUNTIME-LLM/`とする。その外部は当該ターンの明示許可なく触れず、外周境界内も当該ターンで許可されたRoot／Path以外を自動許可しない。`other/`はユーザー専用領域として通常Authorityから除外し、明示的な一時解除がない限り一切触れない。未許可DirectoryへCopy Folder、Temporary Artifact、Stage、CacheまたはBackupを作成しない。
...
Branch Protection、Phase 1-ex完了Tag、Releaseおよび単一Git Root Cutoverはこの時点では未実施であった。後続状態は第2.26節を優先する。
...
SourceとGit StagingのDelta Refresh、Backup Gate、Actual Integration、旧Directory／廃止Image整理、Publication Set一致、Sanitation、Commit、Draft PR #1、Merge Commit、作業Branch退役、追加DocsのDirect `main` Commit／Pushおよび単一Git Root Cutoverまで完了した。
...
## 2.27 Phase 2 Pilot First／Governance Constitution／Desktop予約
...
- [Documentation Rules](../../shared/conventions/documentation_rules_ja.md)
- [Executable Governance Constitution／Phase 2・3 Pilot Evidence設計記録](history/operations/executable_governance_constitution_and_phase_2_3_pilot_evidence_design_20260804045158.md)
- [Append-only Documentation Index](history/documentation_index_20260804045158.md)
- [Agent／Tool Constitution Enabled Mode予約](history/operations/agent_tool_constitution_enabled_mode_reservation_20260804050816.md)
- [Constitution Mode Documentation Index](history/documentation_index_20260804050816.md)

Phase 2の最初の作業を`Phase 2-0 — Orchestration Pilot Design／Bootstrap`とし、元来のPhase 2-A～2-Fより前に実施する予約をAcceptedした。

現設計統括者役はProject責任者としてProject全体、Cross-Phase不変条件、Task編成、Handoff、ReviewおよびRecoveryを統括する。ユーザーの最終Decision Authorityは維持する。Project責任者も絶対禁止事項、Docs規則、Authority規則その他の運用ルールから免除されず、自己判断でRuleまたはAuthorityを上書きしない。承認・確認待ちの安全な停止は別の正常運用である。

Pilotは一つの有界なWork Unitから始める。必要Taskの作成、Task名設定、Authority設定、Task間Handoff／Status／Follow-up／ReviewをAccepted Envelope内で連結し、安定後だけ複数Unit、Subphase、Phase完了単位へ拡張する。Project完了単位は長期目標であり、自動移行しない。Phase 2を成立性検証とし、Acceptedされた場合はPhase 3で再現性・移植性を検証してから適用範囲をさらに広げる。

Phase 2から、Append-only Documentation Index Snapshotを`docs/project/phases/phase_2/history/index/`へ保存する。Phase 1／Phase 1-exの既存Indexは移動しない。

Agent／Tool本格実装前に、絶対禁止、Docs、Authority、Mutation、Handoff、Review、Recovery、Backup、Git、Costおよび停止規則をProvider-neutralな統合憲法体系へLossless再編する。`docs/project/shared/constitution/`を他Projectへ配置して同等の開発体制を再構築でき、Codex Desktop／Claude Codeの双方へAdapter可能なPackageを目標とする。

統合憲法は単一巨大Markdownではなく、正本Index、章別Rule、Rule ID、Manifest、Role別Constitution View、Schema、AdapterおよびTemplateへ分割する。Phase 2・3で成功だけでなくIncident、Near Miss、人間介入、Ruleの有効性／曖昧性／欠落／過剰制限／検知不能および自動化候補を収集し、Constitution Research Preview、Governance Testおよび改憲へ接続する。

Agentおよび各Toolには、Component本体と分離した`constitution.enabled = ON／OFF`を予約する。ONはAccepted Revision／View／Digestを検証して適用し、OFFは憲法固有処理を行わない研究Baselineとする。OFFでもPlatform Security、Tool Permission、Human Approval、既存AuthorityおよびProject運用ルールは維持する。Agent側ONはTool側ONまたは実行許可を生成せず、ONで必要Artifactを解決できない場合はFail-closedとする。

Desktop Application化は後続Phase予約とし、Phase／技術／配布方式は未決定とする。
...
Stage 1のGit未使用掲載は既存公開Historyとして保持する。Git Staging CloneでそのHistoryを継承し、Original→Clone Integration、Commit、Push、Draft PR、Merge Commit、`main` Postflight、追加Docs Commit／Pushおよび単一Git Root移行まで完了した。新しいRoot Commit、Repository再作成、History RewriteまたはForce Pushは行っていない。今後は[Git Workflow Policy](../../shared/operations/git_workflow_policy_ja.md)に従い、Phase Final Backupと完了Tag候補を独立したユーザーGateとして扱う。
...
- [Git Source→Target Integration／Publication／Single Root Cutover](history/operations/git_source_target_integration_publication_and_single_root_cutover_20260804035722.md)
- [Interim Design Governance Recovery Manifest](../../shared/history/design_governance_handoff/design_governance_recovery_manifest_20260804035722.md)
- [Phase 2 Pilot／Governance Constitution／Desktop Reservation](history/operations/phase_2_pilot_governance_constitution_and_desktop_reservation_20260804043434.md)
- [Git Workflow Policy](../../shared/operations/git_workflow_policy_ja.md)
...
- [2026-08-02 21:34:43](history/documentation_index_20260802213443.md)
- [2026-08-02 22:06:59](history/documentation_index_20260802220659.md)
- [2026-08-02 22:17:49](history/documentation_index_20260802221749.md)
- [2026-08-02 22:36:57](history/documentation_index_20260802223657.md)
