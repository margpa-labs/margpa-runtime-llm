...
Auto-start Project-side Read-only PreflightはRepository実装としてAcceptedである。Lifecycle ScriptsはSafety Follow-up Requiredであり、現在実装許可される範囲はFollow-up HandoffのRepository内ScriptとTestだけである。Public Demo本体、匿名Access、RAG、Dependency変更、Lightning側のFile配置・Secret・Hook・Port設定、Platform変更およびGit操作は含まない。
