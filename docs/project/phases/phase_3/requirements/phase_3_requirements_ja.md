...
- Phase 3機能のLightning Deployment。Phase 2から延期されたLightning横断AcceptanceはPhase 3-Hの別Gateとして扱い、Deployment Authorityと混同しない。
...
Phase 3の最終完了は、Phase 3-HでCodex独立Review、ユーザーMac手動Acceptance、Phase 2から延期されたLightning横断Acceptanceの解決または正式再延期、Final Docs、Backup、Git判断およびユーザーAcceptanceが成立した場合だけ宣言できる。
