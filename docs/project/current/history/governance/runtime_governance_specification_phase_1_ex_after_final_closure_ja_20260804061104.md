...
## 41. Agent／Tool Constitution Enabled Mode

Agentおよび各Toolへ、Component本体の有効状態および通常Governance Bindingと独立したConstitution適用状態を持たせる。
...
- Accepted Constitution Revisionと対象Component用Viewを解決する。
...
- 必須ArtifactまたはCapabilityを解決できない場合はFail-closedとする。

### 41.2 OFF

- Constitution ViewのLoad、憲法固有Evaluationおよび憲法固有Evidence生成を行わない。
- Governance効果の比較、AblationおよびDefinition 0件Baselineと区別可能な研究Baselineとして扱う。
- `allow all`、既存Policyの削除、Tool Permissionの無効化またはHuman Approvalの省略として扱わない。
- Project開発を行うRole／Taskの絶対禁止、DocsおよびAuthority規則には影響しない。

### 41.3 Composition

AgentとToolごとに独立した状態を持つ。Agent Constitution ONでも、呼び出すToolのConstitution状態、Tool Permission、Side Effect ApprovalおよびHuman Approvalを別途解決する。
...
上記を黙って正常扱いしない。`invalid_combination`、`not_applicable`またはFail-closed Errorとして明示する。

### 41.4 Profile／UI

Research／Developer Modeでは、AuthorityとProfileが許可する場合にON／OFF比較を提供できる。一般公開、Productionまたは安全性を固定するProfileではON固定またはToggle非表示にできる。Default値、変更Authorityおよび公開範囲は後続設計で確定する。

Toggleの存在、表示または変更可能性はAuthority昇格手段ではない。
