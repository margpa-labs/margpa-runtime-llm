...
Git／GitHub                   : OPERATIONAL／SINGLE CANONICAL ROOT／MAIN ALIGNED AT 30D347E
...
Documentation RAG: Local Project Corpus／Lightning Public Corpus
...
最上位規則群は現時点の列挙で閉じない。追加候補はEvidence、Conflict、検知、違反時動作、RecoveryおよびUser Decisionを経る。Agent／Tool本格実装前を原則編纂Gateとするが、Rule Conflict、Provider差またはSource肥大化Riskに応じて前倒しできる。
