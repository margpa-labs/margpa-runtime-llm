...
### 25.9 Automation Control State／Pre-pilot Safety要件

Automationの許容範囲を表す`manual／advisory／bounded_unit／workflow／phase／project`等のAutomation Levelと、運用状態を表す`OFF／ARMED／ON／PAUSED／EMERGENCY_STOP`を分離する。

- `OFF`：自動連結を行わない。
- `ARMED`：設計、検証、Backup、AuthorityおよびREADY Evidenceが揃い、後続のユーザーStart宣言を待つ。
- `ON`：Accepted Envelope内だけを自動連結できる。
- `PAUSED`：Resource、Review、利用可能量、外部状態その他の非重大停止。未完了をCompleteにしない。
- `EMERGENCY_STOP`：権限逸脱、許可Root外操作、重大IncidentまたはEvidence不整合。人間の明示判断まで再開しない。

Control TaskのREADY宣言には、少なくともBackup勧告／完了確認、Accepted Authorization Envelope、Authorized Root、禁止事項、Recovery、Resource／Stop条件、最初の有界Work UnitおよびProvider CapabilityのEvidenceを必要とする。READY後にScope、Profile、RevisionまたはCapabilityが変わった場合は失効させる。

将来のPermission／ACL Hardeningは未決定である。AI／Task／Toolが作成したArtifactであっても、対象、Before／After、Platform影響、Lockout Risk、Rollbackおよびユーザー明示承認なしにPermissionを変更しない。

Path Allowlist、許可外DirectoryのRead-only化、隔離Workspace、Mutation Inventory、許可Root外Diff検知およびTool Wrapperは将来の機械的強制候補であり、現時点では未実装・未承認とする。

GitHub Pushと分離した軽量CheckpointをRiskに応じて提案できるが、Local Commit、Patch、Archive、SnapshotまたはManifest Backupの各Mutationは別途ユーザー承認を必要とする。Backupの存在と復元確認を同一視しない。
