...
## 4. 現在地 — Phase 2 Started／Phase 2-0 Automation Pilot Design
...
Phase 2                              : Started
Phase 2-0 Automation Pilot           : Design Complete／Review Pending／Task Not Created
...
Phase 1とPhase 1-exは完了した。Phase 1-exでは、Docs Directory Migration、Canonical／Shared／Public正本、Phase単位Lossless、二種のRecovery Handoff、Git History継承、単一Canonical Git Root、Public Demo、Traffic-aware Auto-start、Mac／Lightning Documentation RAG、公開SanitationおよびPhase Backup契約を整備した。Gitの通常Commit／Push経路は成立済みであり、Git操作そのものは今後も対象ごとのユーザー明示承認を必要とする。Phase 2はユーザー確認により開始し、現在は元来の機能実装に先立つPhase 2-0 Document-driven Orchestration Pilotの設計Review待ちである。Authorization Envelopeは未承認、独立Taskは未作成、Pilotと機能実装は未開始である。
...
**State: `In Progress — Phase 2-0 Pilot Design／Review Pending`**
...
Phase 2は、元来のConversation／Configuration機能へ着手する前に、`Phase 2-0`としてDocument-driven Orchestration Pilotの設計と最小実行を行う。設計統括者役がPhase専用のIndex、開始用Handoff、Reading OrderおよびWrite Authorityを用意し、独立した`Phase 2 設計担当者役`Task等へ直接渡す最初のPilotとする。

現設計統括者役はProject責任者としてProject全体、Cross-Phase不変条件、Task編成、設計／実装Handoff、ReviewおよびRecoveryを統括する。ただし、ユーザーの最終Decision Authority、Backup、Git／公開、External Service、Secret、課金およびPhase移行Gateを代替しない。Project責任者も絶対禁止事項、Docs規則、Authority規則その他の運用ルールから免除されず、自己判断で規則やAuthorityを上書きしない。
...
Pilot開始時は一つのSubphaseまたは有界なWork Unitごとに、Task作成、Task名設定、Authority設定、Handoff、Status、Follow-upおよびReviewを繰り返す。安定性が確認できた場合だけ、複数Unit、Subphase、Phase完了単位へ拡張し、最終的にはProject完了単位のOrchestrationを長期目標とする。粒度拡大は自律性や権限の無条件拡大を意味しない。
