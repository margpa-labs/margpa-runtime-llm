...
document_state: phase_7_bounded_mvp_external_web_deferred
language: ja
created_at: 2026-08-23
updated_at: 2026-08-29 22:26 JST
...
| Phase 7 | Closure整理中 | Local RAG／Citation／Data Governance | Local Corpus、Citation、Data ControlsとWeb Port／Security Scaffoldを実装。実Web検索はPhase 11以降へ延期 |
...
| Phase 10以降 | 将来研究 | Governance完成／Hardening／External R&D | Phase 10でSemantic GovernanceとPADGを扱い、Phase 11以降でGoverned External Web Runtime等へ再分割 |
...
- Selene、Qwen3Guard、GD Semantic Live Evaluation、Judge／Repair Golden PathおよびMain Semantic ENFORCEは新Phase 10へ正式移管した。Phase 7〜9はRule／Pattern Base GuardrailとBuilt-in Deterministic JudgeまたはNone／OFFを暫定Baselineとする。

### Phase 7〜9 — 知識・行動・比較研究

- Phase 7でLocal Corpus、Data Source、Retrieval Evidence、Document Injection、Citationおよびデータコントロールを扱う。Provider非依存Web Search／Fetch Port、Fixture TestおよびSecurity Scaffoldまでは保持するが、実General Web Search、外部Network、Web EvidenceのChat／Citation接続、一般URL Fetchは完成を主張しない。
...
- 新Phase 10冒頭で、Phase 6のProvider／Lifecycle／Budget／Evidence基盤を再利用し、Selene、Qwen3Guard、GD Semantic 109件、Independent Judge／Repair／RejudgeおよびSemantic ENFORCEを差分完成させる。
- 実General Web Search、一般URL Fetch、Web-grounded Chat、外部送信Consent／PII EnforcementおよびHostile-site Sandboxは、Account／Credential／Cost／Privacy／Provider運用を含む`Governed External Web Knowledge Runtime`としてPhase 11以降へ延期する。
...
Phase 3〜5は完了。Phase 6は主要基盤を成立させた一方、MARGPA Semantic Rule 109件、独立Judge／Guardrail Model、Judge／Repair Golden Pathを未解決として保持し、User判断による特殊最小Closureを完了した。Phase 7ではLocal Corpus／Citation／Data Controlsと将来Web Runtime用Port／Security Scaffoldを実装した。実Web検索はSecurity、Privacy、Provider運用および公開Demo Riskを理由にPhase 11以降へ延期し、現在はこの正直なClaim境界でPhase 7 Closureを整理している。
