...
updated_at: 2026-09-07 16:51 JST
...
## 4. 現在地 — Phase 9-1 Complete／Phase 9-2 User Manual Waiting

2026年9月7日時点の現在地は次のとおりである。
...
Phase 9                               : In Progress／Phase 9-1 Complete／Phase 9-2 User Manual Waiting
Phase 9-1 Judge／Repair                : Gemma 4 E2B／32 Criterion／Repair→Rejudge User Mac Passed
Phase 9-1 Qwen3Guard                  : Real OBSERVE／ENFORCE／OFF User Mac Passed
Phase 9-1 Main Governance ENFORCE     : Judge連携Semantic Repair／judge_and_main User Mac Passed
Phase 9-1 Selene                      : Deferred／Known Unresolved／Non-blocking
Phase 9-1 Optional Judge Providers    : Main-shared Qwen／DeepSeek Known Unresolved／Non-blocking
Phase 9-2 Internal Implementation     : Controller Accepted／Backend 2665 Passed／43 Deselected
Phase 9-2 User Mac Manual             : Deferred until Weekly Quota Recovery／Not Yet Run
Phase 9-3                             : Not Started
...
Phase 6の実画面確認では、ARGD／DAGDを含むSemantic Rule 109件が全件`Deferred（意味評価待ち）`となり、独立Judge、Repair／RejudgeおよびMain GovernanceのSemantic ENFORCEも未成立だった。この結果を解決済みと扱わず、Phase 9-1でJudge／Guard／Main Governanceを疎結合なComponentとして再構成した。

Phase 9-1では、Gemma 4 E2BへJudge専用の制約付きJSON生成を適用し、32 Criterionの評価、Judge起点Repair、同一CriterionによるRejudge、Main Governance起点Repairおよび`judge_and_main`を実機Macの通常画面で確認した。Qwen3GuardもOBSERVE／ENFORCE／OFFを通過し、各ComponentのOFF／Unload後に通常Chatへ戻れることを確認した。Selene、Main-shared Qwen Judge、DeepSeek Judge、Guardrailの過敏な分類および一部UI表示は未解決として残るが、Gemmaを用いる必須Golden Pathの成立を妨げないため、Phase 9-1はUser Acceptedとして完了した。
...
Phase 9は9-1／9-2／9-3の3 Programへ分解した。Phase 9-1は複数回の独立Review、実Model ReworkおよびUser Mac Manualを経て完了した。Phase 9-2はExperiment Plan、Variant、Frozen Configuration、非同期Run、Evidence、Comparison、Restart Read、設定Leaseおよび最小UIを実装し、Controller Reviewまで受理済みである。実Model GateもMain Qwen、Gemma Judge／RepairおよびQwen3Guardの対象Scenarioで成立した。残るUser Mac実画面確認は週間利用可能量の回復後へ延期しており、完了前のためPhase 9-2全体は`Validation Waiting`、Phase 9-3は`Not Started`とする。
...
**State: `Implementation In Progress — Phase 9-1 Complete／Phase 9-2 Validation Waiting`**
...
各ProgramはGateまでLong-runし、原則として観点変更二段階自己Review後にCodex Controller Reviewへ返す。Phase 9-1はPreflight、Source実装、Real Model Load、反復Review／ReworkおよびUser Mac Manualを完了した。Phase 9-2も内部実装、Controller Reviewおよび実Model Gateまで成立し、週間利用可能量の回復後に行うUser Mac Manualを残している。Phase 9-3はPhase 9-2のUser Acceptance後まで開始しない。
...
専用ModelのHardware／Resource／Lifecycle Failureは正直な途中状態として記録し、Built-in／Rule-based／Noneや`RESOURCE_GATED`を必須Golden Pathの代替にしない。一方、特定の候補Modelだけに固有の不具合を、交換可能な別Providerで基盤が成立した後も無期限のBlockerにはしない。Phase 9-1ではGemmaを独立Judgeとして採用し、Qwen3Guard、Semantic Criterion評価、Main Semantic ENFORCEおよびJudge→Repair→RejudgeをUser実画面で成立させた。OFF後の通常Chat復帰も確認済みである。

Phase 9-1の最終User Mac Manualで確認したDispositionは次のとおりである。

- Gemma Judge単独OBSERVE、Judge起点Repair／Rejudge、Main Governance起点Repair／Rejudgeおよび両者が要求する`judge_and_main`が成立した。
- 通常選択32 Criterionを評価し、残る77件をDeferredとして109件のCount保存則を維持した。Gemmaの制約付きJSON生成後は、対象実画面試験で`malformed_output`／`unavailable`が再発しなかった。
- Qwen3GuardはReal ArtifactでOBSERVEの検知・非介入、ENFORCEの拒否、OFF後の非Active化を確認した。
- Main Governance／Judge／Guard／Repairは独立Modeとして操作でき、Main Governance OFFでもDedicated Judgeが実行できる。各ComponentをOFFに戻した後は通常Chatを継続できる。
- Selene、Main-shared Qwen JudgeおよびDeepSeek Judgeは必須構成から外し、交換候補の未解決事項として保持する。
- 現行Main GovernanceのSemantic ENFORCEはJudgeによる意味評価と連携する。Judgeなしで直接Modelを矯正するStructural Layerは、Phase 11以降の別設計であり、Phase 9-1の成立範囲へ含めない。
...
Phase 9-2では、Experiment Plan、Case、Variant、RunおよびRequestのIdentityを分離し、次の最小Research Runtimeを実装した。
...
Plan作成時のDesired Configurationと実行時のFrozen Configurationを保存し、VariantごとのProvider Identity、Component Call、Raw Evidence、比較結果およびRestart後の読取を追跡する。非同期Worker、Deadline、Cancel、Terminal Resultの一回Publish、Live Configuration Leaseおよび設定不一致のCall 0 Rejectも備える。FixtureとProductionを明示的に区別し、未観測値を成功またはCall 0へ変換しない。

Controller ReviewではBackend非Model Suite `2665 passed／43 deselected`、Ruff Cleanおよび既存Mypy Baseline維持を確認した。Main Qwenのみ、Main Qwen＋Gemma Judge／Repair ENFORCE、Main Qwen＋Qwen3Guard ENFORCEのTop-level実Model Scenarioも`completed`へ到達した。現在は、比較UI、同一Plan内の複数Variant、Restart Readおよび設定不一致Rejectを実画面で確認するUser Manual Gate待ちである。
...
Phase 9-1は`Complete／User Accepted`である。Phase 9-2は内部実装、Controller Reviewおよび実Model Gateまで成立し、User Mac Manualだけを残す`Validation Waiting`である。週間利用可能量の回復まで手動確認を延期し、結果受領前にPhase 9-2 Complete、Phase 9-3開始、Phase 9 ClosureまたはPhase 10 READYをClaimしない。
