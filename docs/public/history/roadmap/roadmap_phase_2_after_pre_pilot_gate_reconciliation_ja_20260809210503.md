...
## 4. 現在地 — Phase 2 Started／Phase 2-0 Pre-activation Gate

2026年8月9日時点の現在地は次のとおりである。
...
Phase 2                              : Started
Phase 2-0 Automation Pilot           : Gate Reconciled／Pre-activation Ready Candidate／Task Not Created
...
Phase 1とPhase 1-exは完了した。Phase 1-exでは、Docs Directory Migration、Canonical／Shared／Public正本、Phase単位Lossless、二種のRecovery Handoff、Git History継承、単一Canonical Git Root、Public Demo、Traffic-aware Auto-start、Mac／Lightning Documentation RAG、公開SanitationおよびPhase Backup契約を整備した。Gitの通常Commit／Push経路は成立済みであり、Git操作そのものは今後も対象ごとのユーザー明示承認を必要とする。Phase 2はユーザー確認により開始し、元来の機能実装に先立つPhase 2-0 Document-driven Orchestration PilotのGate順序、Capability Preflight、Envelope RevisionおよびBootstrap Handoffを再照合した。本更新を含むCommit／PushがRemote一致まで通過すれば、技術的な開始準備で残るのはユーザーによる大規模Backupである。正式なActivationには、その後もExact Envelope／Task範囲Acceptanceと両者合意によるREADY／Startを必要とする。Authorization Envelopeは未承認、独立Taskは未作成、Pilotと機能実装は未開始である。
...
**State: `In Progress — Phase 2-0 Pre-activation Gate／Checkpoint Transaction`**
...
Phase 2はユーザー確認により開始済みである。ただし、Phase 2開始とTask作成は分離する。Phase 1-exの完了、User AcceptanceおよびBackupは成立済みだが、Authorization Envelopeの明示承認とユーザーの明示的なTask作成指示は未成立である。ユーザーが、Task作成、要件変更、重要な外部操作、Backup、Commit／Push／公開、User AcceptanceおよびPhase移行の最終Authorityを保持する。
...
Pilot開始前にユーザーが大規模Backupを取得する。その後、Control Taskの「準備OK。いつでも開始出来ます。」と、後続ユーザーの「ok。では開始する。」が順序どおり成立した時だけPilotを一斉開始し、Task名変更を行う。片方の発言、過去の同意または類似表現から開始を推測しない。

初回PilotのGate順序は、Design Review、Read-only Capability Preflight、Exact Design Freeze、ユーザー承認済みGit Checkpoint／Remote一致、ユーザー大規模Backup、Exact Envelope／Child Task範囲Acceptance、Controller READY／`ARMED`、後続ユーザーStart／`ON`、Child Task作成、Acknowledgement／Reviewへ固定する。2026年8月9日のRead-only Preflightでは、Task作成、Task名設定、初回Handoff、Follow-up、状態取得およびWaitを確認し、Interruptは`manual_required`とした。Taskはまだ作成していない。
