...
updated_at: 2026-08-29 22:26:47 JST
...
Phase 7                               : Bounded MVP Implemented／Local Corpus・Citation・Data Controls／Real Web Deferred
...
Judge／Repair／Rejudge Golden PathおよびMain Semantic ENFORCEは、新Phase 10の
`Governance Semantic Runtime Completion Program`へ正式移管する。Phase 7〜9では、既に成立した
Built-in Rule／Pattern Base GuardrailとBuilt-in Deterministic Judgeまたは`None／OFF`をPoC Baselineとして
使用し、Dedicated Modelや未評価Semantic Criterionを実行済みまたはPassと表記しない。

---

## 13. Phase 7 — RAG and Data Governance

**State: `Bounded MVP Implemented／External Web Deferred to Phase 11+／Closure Preparation`**
...
実General Web Search、一般URL Fetch、Web EvidenceのMain Model／Citation接続、Server Canonical OFF、
外部送信Consent／PII EnforcementおよびHostile-site SandboxはPhase 11以降へ延期する。Phase 7では
...
### 16.0 Governance Semantic Runtime Completion Program

Phase 6で成立したProvider Registry、Role Lifecycle、Budget、Deadline、Cancel、Recording、Failure Presentation、
Built-in Evaluator／Rule-based GuardおよびGD Compiler入口をAs-built Baselineとして再利用し、次を完成させる。

- Selene-1-Mini-Llama-3.1-8B Dedicated Judgeの実Artifact Load／Inference／Prompt／Strict Output Contract
- Qwen3Guard-Gen-0.6B Dedicated Guardの実Artifact Load／Inference／Target別Output Contract
- ARGD／DAGDその他GD Semantic RuleのLive Criterion EvaluationとCriterion Evidence
- Built-in／Dedicated／Noneを比較できるJudge／Guard Provider構成
- Independent JudgeによるJudge／Repair／Rejudge／Presentation Golden Path
- Main Governance Semantic ENFORCE、Conflict／Priority／Budget
- Configured／Active／Executed／Evidence IdentityのReal Browser一致
- Hardware／Latency／Memory／Model同時常駐Profileの実測

Phase 6 Sourceをゼロから再実装せず、未解決RegistryとPhase 6 Difficulty Retrospectiveから差分再開する。
18系統GDおよび本格Constitutionを一度に全ENFORCEせず、Foundation、OBSERVE、限定ENFORCEの順で検証する。
...
### 16.14 Phase 11以降 — Governed External Web Knowledge Runtime
...
Manual Evidence SelectionとOne-shot Chat Groundingを先に成立させ、Automatic Search Triggerは別Gateとする。一般URL FetchはModel自身の能力ではなくGoverned Backend Actionとして扱い、未知・攻撃的Site、Archive／PDF／Mediaおよび認証領域はSandbox／Parser Isolation成立後の別Tierに分離する。
